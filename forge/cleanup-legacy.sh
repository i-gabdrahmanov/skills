...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --ext|--src)   [ $# -ge 2 ] || { echo "cleanup-legacy.sh: --ext/--src без пути" >&2; exit 2; }
                   EXT="$2"; shift 2 ;;
    -h|--help)     sed -n '2,38p' "${BASH_SOURCE[0]}" | sed 's/^# \{0,1\}//'; exit 0 ;;
    -*)            echo "cleanup-legacy.sh: неизвестный аргумент: $1" >&2
                   echo "  Usage: bash cleanup-legacy.sh [<проект>…] [--apply] [--purge-state] [--backup-dir <путь>] [--ext <путь>/forgeExt]" >&2
...
resolve_ext() {
  local cand src b
  if [ -n "$EXT" ]; then                       # явный --ext/--src
    EXT="$(cd "$EXT" 2>/dev/null && pwd)" || { echo "cleanup-legacy.sh: нет каталога --ext" >&2; exit 2; }
    is_ext_root "$EXT" && return 0
    echo "cleanup-legacy.sh: --ext $EXT не похож на forge (нет skills/*/SKILL.md и hooks/*.py)" >&2
    exit 2
  fi
  ...
  echo "cleanup-legacy.sh: не нашёл каталог forge — а без него не понять, что форжевое," >&2
  echo "  и чистка сняла бы лишь часть (это хуже, чем ничего). Скрипт лежит в $EXT_SELF." >&2
  echo "  Укажи корень явно: --ext <путь>/forgeExt (или запусти из каталога forgeExt/)." >&2
  exit 2
}
TS="$(date +%Y%m%d-%H%M%S)"
