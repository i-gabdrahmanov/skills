...
# Не «первый из PATH», а первый ЗДОРОВЫЙ: версия >= пола и живой stdlib (expat).
# Тот же выбор запекается в settings.json как ${PYTHON} для каждого хука, поэтому
# кривой интерпретатор здесь = молча снятый enforcement. См. hooks/_pick_python.sh.
