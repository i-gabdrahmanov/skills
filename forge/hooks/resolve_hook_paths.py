...
def find_project_root(cwd: str | None = None) -> str:
    """Определяет корень проекта через git, gradle или ground/<policy.json|pipeline.json>.

    Backwards-compat wrapper: delegates the resolution algorithm to
    :func:`_config_loader.find_project_root` (Phase 0 v2 consolidation) and
    coerces the result to ``str``. Falls back to ``str(start)`` on miss so
    callers that build ``${PROJECT_ROOT}``-substitutions always have a
    non-empty string to splice into the settings.json template.
    """
    ...
    if not start.is_absolute():
        start = start.resolve()
    return str(_canonical_find_project_root(start) or start)
