...
Группировка (гибрид, стабильная по сессии):
...
Каталог прогона фиксируется ОДИН РАЗ на session_id через pointer-файл
<root>/ground/ai-logs/.run-index/<session8>.path — поэтому ad-hoc метка времени не
«плавает» поминутно и вся сессия пишет в один каталог (раньше %H%M пересчитывался на
каждом событии и размазывал одну сессию по десяткам каталогов). Как только в прогоне
появляется манифест, события уходят в <feature>/iter-<iter>.
...
# Общий resolver путей/фич (тот же, что использует state-recorder и остальные хуки).
sys.path.insert(0, str(Path(__file__).resolve().parent))
from _project import active_feature
...
def _active_manifest(root: str) -> dict:
    """dict свежайшего не-archived manifest pipeline-state, либо {} если нет."""
...
def _pointer_path(base: str, session: str) -> str:
    return os.path.join(base, ".run-index", f"{session}.path")


def _read_pointer(base: str, session: str) -> str | None:
    try:
        with open(_pointer_path(base, session), encoding="utf-8") as f:
            p = f.read().strip()
        return p or None
    except Exception:
        return None
...
def _write_pointer(base: str, session: str, run_dir: str) -> None:
    """Атомарная запись pointer (tmp + os.replace), чтобы конкурентные процессы
    субагентов не повредили файл. Никогда не бросает наружу."""
    try:
        d = os.path.join(base, ".run-index")
        os.makedirs(d, exist_ok=True)
        fd, tmp = tempfile.mkstemp(dir=d, prefix=f"{session}.", suffix=".tmp")
        try:
            with os.fdopen(fd, "w", encoding="utf-8") as f:
                f.write(run_dir)
            os.replace(tmp, _pointer_path(base, session))
        finally:
...
def _run_dir(root: str, data: dict) -> str:
    ...
    # 1. Активный пайплайн (общий резолвер фичи) → <feature>/iter-<iter>.
    feature = active_feature(Path(root))
    if feature and feature != "pipeline":
        man = _active_manifest(root)
        ...
        run = os.path.join(base, _safe(feature), "iter-" + _safe(it))
        if sess != "nosess":
            _write_pointer(base, sess, run)
        return run

    # 2. Pre-manifest: переиспользуем уже зафиксированный для сессии каталог.
    if sess != "nosess":
        existing = _read_pointer(base, sess)
        if existing:
            return existing

    # 3. Первое ad-hoc событие сессии: фиксируем метку времени один раз.
    stamp = datetime.now().strftime("%Y%m%d-%H%M")
    run = os.path.join(base, "_adhoc", f"{stamp}-{sess}")
    if sess != "nosess":
        _write_pointer(base, sess, run)
    return run
