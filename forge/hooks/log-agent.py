...
(раньше ключевались по новейшему манифесту + iter-<N> и логи одного прогона разъезжались по
 <feature>/iter-* и _adhoc/* — см. _run_dir).
...
def _run_dir(root: str, data: dict) -> str:
    """Каталог прогона: ОДИН на сессию — «один прогон = одна папка / один таймлайн».

    Раньше ключевались по «самому свежему манифесту ЛЮБОГО скилла» + iter-<N>, поэтому логи одного
    прогона разъезжались: до init pipeline-state — в `_adhoc/<ts>-<sess>`, при запуске субагента
    system-analyst (его манифест становился новейшим) — в `system-analysis/iter-*`, затем — в
    `<feature>/iter-*`. Пользователь справедливо удивлялся «почему в разных файлах».

    Теперь стабильный ключ = `session_id`: все события прогона (главный агент + субагенты + до/после
    init) пишутся в `ai-logs/run-<session8>/`. Фича по-прежнему видна в манифесте; кросс-прогонный
