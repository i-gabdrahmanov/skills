...
Самая критичная находка Windows-аудита: cost-breaker.py и log-agent.py безусловно
...
test_log-agent.py/test_cost-breaker.py) — иначе моки в sys.modules теста не видны
дочернему процессу.
"""
...
class TestCostBreakerFallback(unittest.TestCase):
    def test_loads_without_fcntl_and_uses_msvcrt(self):
        fake = _FakeMsvcrt()
        mod = _load_module_without_fcntl("cost_breaker", HOOKS / "cost-breaker.py", fake)
