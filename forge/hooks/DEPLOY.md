...
действия. Логгер — всегда последний и неблокирующий. Точный блок — в `settings.hooks.json`.
...
| Каталог | Зачем |
|---|---|
| репо Forge (этот каталог — родитель `hooks/`) | **source-of-truth**: `hooks/` + `skills/` + `deploy.sh` + `uninstall.sh` |
...
Снять обвязку (зеркало деплоя, те же аргументы; `--dry-run` — план, `--purge-state` — снести ещё
и `ground/` с git-чекпойнтами `refs/forge/*`):
```bash
bash uninstall.sh /path/to/target-project
```
`uninstall.sh` снимает блок `hooks` **до** удаления файлов и уносит только своё: `ground/`,
`permissions`/`mcpServers`, чужие хуки оператора и бэкапы остаются.
...
>
> ⚠️ **И не удаляй `.gigacode/` руками** — зеркальная поломка: файлов нет, а блок `hooks` в
> `settings.json` остался → рантайм зовёт удалённые скрипты и падает на КАЖДОМ вызове инструмента.
> Для снятия есть `uninstall.sh`.
