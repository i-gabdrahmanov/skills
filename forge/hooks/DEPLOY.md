...
> домашнего `~/.gigacode`**. Деплой = положить/обновить `.gigacode/` в репозитории (например,
> `deploy.sh <project>/.gigacode`), а не в домашний каталог. Домашний дом, если используется, —
> лишь место для бинаря форка, но код пайплайна читается из проекта.
