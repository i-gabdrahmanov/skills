...
| `fork-syntax-guard.py` | PreToolUse `^Bash$` | инструктивный блок синтаксиса, который режет нативный сейфти форка (`$(...)`, backticks, `find -exec`, `ls -R`) — объясняет замену (Glob/Grep/Read) вместо молчаливого deny | exit 2 |
...
PreToolUse `^Bash$` идёт **sequential**: destructive-blocker → fork-syntax-guard → pii-boundary → cost-breaker →
