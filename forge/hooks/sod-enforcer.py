...
try:
    ...
    try:
        ...
        print(f"[sod-enforcer] DENY: бандл forge не грузится ({_e}). Проверь интерпретатор в "
        ...
        sys.exit(2)
    sys.exit(bundle_denied("sod-enforcer", _e))
...
def _active_step_id(root: Path) -> str | None:
    ...
    резолвер уже использует gate-guard. Fallback на in_progress — если шаг не разрешился."""
    try:
