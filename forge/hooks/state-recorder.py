...
def main() -> int:
    try:
        ...
        # Закрываем границу субагента (её открыл context-injector на SubagentStart). Снимаем
        # ДО всей остальной обработки: даже если разбор JSON-выхода ниже уйдёт в return,
        # маркер обязан быть снят, иначе actor-гейт останется выключенным до конца сессии.
        try:
            import subagent_scope
            subagent_scope.leave(str(data.get("session_id") or ""),
