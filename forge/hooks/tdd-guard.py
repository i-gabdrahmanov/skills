...
try:
    ...
    try:
        ...
        print(f"[tdd-guard] DENY: бандл forge не грузится ({_e}). Проверь интерпретатор в "
        ...
        sys.exit(2)
    sys.exit(bundle_denied("tdd-guard", _e))
...
def main() -> int:
    ...
    # (где есть только policy.json) возвращает {} и quality.{test_layer,block_jpa_test,tdd,tdd_integration_skip}
    # не работают. cfg также передаётся в _task_exempt / _all_tasks_exempt ниже — load_project_config
    # даёт ту же структуру dict, поэтому сигнатуры помощников не меняются.
    ...
    # Проверка стояла до фильтра пути и срабатывала на любой файл, где аннотация просто
    # УПОМЯНУТА: `docs/tech-design.md` с фразой «@SpringBootTest поднимает контекст»
    # блокировался наравне с настоящим интеграционным тестом (tasks/011).
    if "src/test/" in target_str:
        ...
        if has_jpa_annotation and block_jpa and test_layer != "mixed":
            return _block(
                ...
                "(падают initializationError). Разреши командой config.py: "
                "config.py --project <root> set quality.block_jpa_test false "
                "(или quality.test_layer mixed) — прямая правка policy.json/pipeline.json "
                "заблокирована state-write-guard."
            )
