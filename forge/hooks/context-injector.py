...
def main() -> int:
    try:
        ...
        # Граница «работа идёт внутри субагента»: agent_type/agent_id рантайм отдаёт ТОЛЬКО
        # на SubagentStart/SubagentStop, в PreToolUse их нет (замерено на qwen-code 0.21.14).
        # Без этой отметки inline-phase-guard принимает субагента за оркестратора и блокирует
        # ровно ту работу, которую сам же требует делать субагентом. См. subagent_scope.
        try:
            import subagent_scope
            subagent_scope.enter(str(data.get("session_id") or ""),
