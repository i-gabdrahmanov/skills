...
#
# Forge стоит EXTENSION'ом (qwen/gigacode)? Тогда этот скрипт не про тебя: раскладки внутри
# проекта у extension'а нет, а мешают ему остатки старого деплоя на user-уровне
# (~/.qwen/skills, ~/.gigacode/hooks, блок hooks в settings.json). Их снимает
# `forgeExt/cleanup-legacy.sh` (план по умолчанию, переносит в бэкап, чужое не трогает).
#
