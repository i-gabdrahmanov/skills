...
со спецификацией в .md** (там должен быть `.git`). Если файла ещё нет — считай его пустым
(`{"projects": {}}`, шаблон — `config.json.example` рядом) и настрой через вопрос
пользователю, как при отсутствующей записи (см. README, сценарий «Первый запуск»).
