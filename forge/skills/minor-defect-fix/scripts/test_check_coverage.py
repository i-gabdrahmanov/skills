...
def main():
    ...
    # 9. Раскладка путей: git отдаёт пути ОТ КОРНЯ РЕПО. В одномодульном проекте это
    #    "src/main/java/..." без ведущего слэша, и прежний фильтр `"/src/main/" in f`
    #    отбрасывал ВСЕ файлы: changed=0 → гейт возвращал pass независимо от покрытия.
    #    Тихий фейл-опен на любом одномодульном репозитории (найдено e2e).
    sys.path.insert(0, str(SCRIPT.parent))
    import check_coverage as cc

    def _filter(paths):
        return [f for f in sorted(paths)
                if f.endswith(".java") and not cc._IS_TEST_PATH.search(f)
                and cc._IS_SRC_MAIN.search(f)]

    check("одномодульная раскладка: src/main/... виден",
          _filter(["src/main/java/com/acme/A.java", "README.md"]) == ["src/main/java/com/acme/A.java"])
    check("многомодульная раскладка по-прежнему работает",
          _filter(["service/api/src/main/java/B.java"]) == ["service/api/src/main/java/B.java"])
