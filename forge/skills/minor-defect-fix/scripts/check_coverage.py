...
# git отдаёт пути ОТ КОРНЯ РЕПО: в одномодульном проекте это "src/main/java/...", без
# ведущего слэша. Прежний фильтр `"/src/main/" in f` совпадал только в многомодульной
# раскладке ("service/api/src/main/..."), а на одномодульном проекте отбрасывал ВСЕ файлы:
# changed=0 → гейт покрытия возвращал `status: pass` независимо от реального покрытия.
# Тихий фейл-опен именно того гейта, ради которого фаза 05-tests и существует.
# Ровно этот класс уже чинили в eval-guard._is_src_main — здесь он дожил до e2e-прогона.
_IS_SRC_MAIN = re.compile(r"(?:^|/)src/main/")
_IS_TEST_PATH = re.compile(r"(?:^|/)src/test/|(?:^|/)test/")
...
def _changed_java(root: Path, base: str) -> list[str]:
    ...
    return [f for f in sorted(files)
            if f.endswith(".java") and not _IS_TEST_PATH.search(f) and _IS_SRC_MAIN.search(f)]
...
def main() -> int:
    ...
    ap.add_argument("--exclude", action="append", default=None,
                    help="glob путей, исключаемых из проверки покрытия (repeatable). Слои, "
                         "непокрываемые текущей тест-стратегией (repository/entity/dto/config при "
                         "service-unit), иначе coverage-гейт требует невозможного и конфликтует с tdd-guard.")
    ...
    # Исключаем слои, непокрываемые текущей тест-стратегией (repository/entity/dto/config при
    # service-unit). Иначе coverage-гейт требует покрыть, напр., JPA-репозиторий, который под
    # service-unit нельзя тестировать (@DataJpaTest блокирует tdd-guard) — гейты конфликтуют.
    excludes = args.exclude or []
    excluded = [c for c in changed if any(fnmatch.fnmatch(c, g) for g in excludes)]
    changed = [c for c in changed if c not in excluded]
