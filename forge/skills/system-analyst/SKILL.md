...
- В `<project>/ground/pipeline.json` задан `docs.docs_path` (и `docs.mode` = in-repo/separate-repo)
...
> через `skill_paths` (из `ground/pipeline.json`). При `docs.mode=separate-repo` `<docs_path>` обязан
