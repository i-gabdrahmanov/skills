...
> **Все пути — в `feature-pipeline/references/skill-paths.json` (секция `skills.system-analyst`).**
...
- В `<project>/ground/policy.json` задан `docs.docs_path` (и `docs.mode` = in-repo/separate-repo); legacy `pipeline.json` тоже читается через dual-read
  для текущего проекта — именно отсюда `skill_paths` резолвит каталог доков и скана. Если нет —
  задай через config-helper (`config.py set docs.docs_path ... --confirm`), не правь JSON руками.
...
> При `docs.mode=in-repo` можно **опустить `-o`** — скрипт сам резолвит канонический путь скана
> через `skill_paths` (из `ground/policy.json`; legacy `pipeline.json` тоже читается). При `docs.mode=separate-repo` `<docs_path>` обязан
> быть **абсолютным** путём спека-репо, иначе scan ляжет не туда.
...
(use-cases, glossary, operations) получают **компактные excerpt'ы релевантных коллекторов**
(см. §0.5.4, `read.py --excerpt-of`), а **не** полный scan-JSON — это экономит токены и
не перегружает их контекст. Каждый субагент получает: путь к корню, список модулей, скоуп
и **относящийся к нему срез scan-JSON** (для механических обогатителей api/domain/async —
полный срез своей категории, без усечения).
...
аннотации, модуль. Пути резолвятся из String-констант (вкл. конкатенацию и кросс-файловые
константные классы). Эндпойнты из абстрактного базового контроллера приписываются
наследнику с его базовым путём. Возвращает JSON с массивом endpoints.

### 3.2 async-mapper
Грепает `@KafkaListener`, `KafkaTemplate.send`, `@SendTo`, `@StreamListener`, а также
функциональные биндинги spring-cloud-stream (`Consumer`/`Function`/`Supplier` @Bean при
наличии `org.springframework.cloud.stream`), `@RabbitListener`, `RabbitTemplate`.
Для каждого: topic/queue, message type, направление (producer/consumer), модуль.
Консьюмер с неразрешимым топиком всё равно считается (топик помечается `<unresolved>`).
...
Грепает `@FeignClient`, `WebClient`, `RestTemplate`, `RestClient` (Spring 6.1+),
`OkHttpClient`, gRPC (`@GrpcClient`, сгенерированные `*Stub`), внешние SDK-клиенты
...
сверяет reported со scan и эту дыру не видит). Предупреждение `⚠ сканер недосчитал` —
повод разобраться/прогнать полный `system-analyst` заново, даже если HARD-категории
формально `pass`; шаг `99-verify` при этом не закрывай. (Тот же паттерн — в
`project-grounder` §grounding.)

- exit 0 / `pass` — полнота HARD-категорий подтверждена.
- exit 2 / `fail` — артефакты потеряны. Гейт печатает `missing N: <имена>` (например
  пропущенные entity). **НЕ закрывай шаг 99.** Недостающие HARD-артефакты добирай
  **механически из scan-JSON `items[]`** (они полные) на шаге сборки MD (§4) — это
  entity/endpoint/consumer, их источник scan, **не** синтезаторы; ре-ран
  use-cases/glossary/operations тут ничего не добавит. Прогони гейт снова, пока не `pass`.
