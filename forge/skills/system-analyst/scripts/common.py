...
# Маркеры корня проекта для ДАННЫХ — берём из hooks/_config_loader (Phase 0 v2 refactor).
# Скилл изолирован от hooks/_project (нет кросс-скилловых импортов), но hooks/ лежит рядом
# с skills/ в одном репо, поэтому можем дотянуться до _config_loader через sys.path.
# Список маркеров — ТОЛЬКО policy.json + pipeline.json: новые v2-проекты ходят по policy.json,
# легаси v1 — по pipeline.json. Источник один для всех хуков и скиллов.
...
def _is_ground_marker(p) -> bool:
    """ground/<policy.json|pipeline.json> — то же, что hooks/_project.find_project_root
    третьим приоритетом. Локальный копирайт нужен: импортировать из hooks/_project мы
    сознательно не хотим (как и весь _project — скилл изолирован)."""
...
def find_project_root(cwd: Path | None = None) -> Path:
    ...
    Отличается от repo_root(): тот резолвит git-базу сканируемой системы, а этот —
    корень, где живут ground/ и docs/. Вызов из постороннего каталога без `--project`
    раньше заставлял искать ground/ рядом с cwd и ронял резолв политики.

    Backwards-compat wrapper: delegates the algorithm to
    :func:`_config_loader.find_project_root` (Phase 0 v2 consolidation) and
    keeps the ``or start`` fallback so callers that always expected a non-None
    ``Path`` continue working. Behaviour identical to the pre-refactor inline
    implementation.
    """
...
def iter_files(root: Path, suffixes: tuple[str, ...]) -> Iterable[Path]:
    """Обход исходников с early-пропуском каталогов (НЕ заходя в них).

    Вместо root.rglob(\"*\") (который рекурсивно обходит build/, target/, .git/ и т.д.
    После чего отфильтровывает) — ручной os.walk с prun'ом: пропускаемые каталоги не
    обходим вовсе. На моно-репо с 50 модулями rglob по build/ таймаутил ensure_inventory
    (п.5 KIDPPRB-9254); os.walk с topdown=False-фильтром доходит за секунды.
    """
    root = root.resolve()
    for dirpath, dirnames, filenames in os.walk(str(root), topdown=True):
        dp = Path(dirpath)
        # prunу: удаляем из dirnames пропускаемые каталоги, чтобы os.walk не спускался в них
        kept = []
        for d in list(dirnames):
            if is_skipped_dir(d):
                continue  # не спускаемся в .git/build/target/ground и т.п.
            kept.append(d)
        dirnames[:] = kept
        # для файлов: фильтр по суффиксу (и исключение test-source — по-прежнему через прежний путь)
        if not suffixes:
            for fn in filenames:
                p = dp / fn
