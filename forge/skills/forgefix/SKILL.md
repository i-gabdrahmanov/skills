...
> при развёртывании через `deploy.sh`; см. INSTALL.md). preflight печатает его в `layout.base`;
> ниже любой путь `<project>/.gigacode/skills/...` читай как `<forge>/skills/...`.
...
> общий на все проекты; артефакт задачи, записанный туда, уедет в
...
```
python3 <project>/.gigacode/skills/config-helper/scripts/config.py --project <toplevel> set inputs.story <STORY|none> --skill forgefix --feature <KEY|slug>
```
...
`config.py set inputs.story <...> --skill forgefix --feature <KEY|slug>` + перезапуск.

> **Проверь, что ответ РЕАЛЬНО записан — по exit-коду `set`, а не «по ощущению».** `0` = записано;
> `3` = нет `ground/policy.json` или активного `manifest.json` (вернись к §1.1 и инициализируй
> конфиг/манифест, потом повтори `set`). Это самый частый способ потерять ответ: вопрос задан,
> пользователь ответил, `set` тихо упал. Форсится двумя гейтами: `update.py` не закроет
> `fix-intake` с незаписанным `inputs.story`,
...
- иначе — **лестница вопросов, от дешёвого к точному**:
  1. стори на §2.1 ответили «не знаю» — спроси её ещё раз, теперь уже зная место правки
     ...
     ```
     python3 <project>/.gigacode/skills/config-helper/scripts/config.py --project <toplevel> set inputs.story <KEY> --skill forgefix --feature <KEY|slug>
...
```
python3 <project>/.gigacode/skills/config-helper/scripts/config.py --project <toplevel> set inputs.spec_anchor <REQ-ID|none> --skill forgefix --feature <KEY|slug>
```
Пока `inputs.spec_anchor` не записан, `gate-guard` заблокирует запись дельты на фазе `fix-spec`
(fail-closed). Вопрос не отрендерился (headless/форк) — НЕ угадывай: остановись и попроси
предзапись `config.py set inputs.spec_anchor <...> --skill forgefix --feature <KEY|slug>` + перезапуск.
...
ЯКОРЬ (зафиксированное решение inputs.spec_anchor): <REQ-ID и название | none>.
...
- Не подставлять стори «по догадке»: `inputs.story` — ответ пользователя (или честное `none`).
...
- **Per-feature (`ground/statements/forgefix/<KEY|slug>/manifest.json`)** — входы и решения
  текущего фикса: `inputs.mode`/`inputs.story`/`inputs.spec_anchor` и
  `decisions.mode_task`/`decisions.criticality`/`decisions.auto_max_risk`. Без активного
  манифеста `set inputs.*`/`decisions.*` падает (exit 3).
