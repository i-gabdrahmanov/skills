...
> вся работа идёт в выбранном оркестраторе. Запуск харнеса: `gigacode --experimental-hooks`
> (интерактивно) → команда `/forge <задача>`. Headless `-p` требует `-y`/YOLO и предзаписи решений,
> иначе `agent` не стартует и фаза упирается в `inline-phase-guard` (INSTALL.md §4).
