...
# модуль (нет approval_path и др. → ImportError при сборе pytest в определённом порядке).
...
_RECORD_APPROVAL_SCRIPT = Path(__file__).resolve().parent / "record_approval.py"


class StepGateBlocked(RuntimeError):
    """Шаг НЕ закрывается, потому что не выполнено требование гейта.

    Отдельный тип нужен, чтобы отличить осознанный отказ от настоящей поломки скрипта.
    Все семь гейтов закрытия раньше бросали голый RuntimeError, main() звался без обёртки —
    и рантайм получал ТРЕЙСБЕК с rc=1. Для модели это выглядит как «скрипт сломан»
    (она лезет чинить update.py вместо того, чтобы выполнить требование), а сам текст
    отказа оказывается в ХВОСТЕ stderr, после стека. Плюс rc=1 неотличим от реального
    краха (битый JSON, нет файла), так что оркестратор не может ветвиться по коду.
    Теперь: чистое сообщение в stderr и rc=2 (как у deny-хуков), стек — только у
    настоящих ошибок.
    """
...
def _override_hint(judge: str, feature: str, step_id: str, why_ph: str = "<обоснование>") -> str:
    ...
    return (
        ...
        f"   2) зафиксируй согласие СКРИПТОМ (рукописный маркер не считается — прямая "
        f"запись в approvals/ заблокирована state-write-guard, а gate-guard требует "
        f"провенанс record_approval):\n"
        f"      python3 {_RECORD_APPROVAL_SCRIPT} --key gate-override-{judge} "
        f"--approved-by user --reason \"{why_ph}\";\n"
...
def _check_subagent_origin(step: dict, closed_by: str, project: Path, skill: str, feature: str):
    ...
    raise StepGateBlocked(
        ...
        f"Флаг --closed-by доказательством НЕ считается: его подставляет и оркестратор. Прогони фазу через agent(subagent_type=...) — "
...
def _max_step_reopens(project: Path) -> int:
    """Лимит переоткрытий шага: quality.max_step_reopens из ground/policy.json (v2, дефолт 3).
    Dual-read fallback на legacy ground/pipeline.json — см. hooks/_config_loader.load_project_config."""
    try:
        v = int(load_project_config(project).get("quality", {}).get(
...
def _config_get(project: Path, dotpath: str, skill: str | None = None,
                feature: str | None = None):
    """Читает значение по dot-path.

    Приоритет (v2):
      1. manifest.json активной фичи (если dotpath в inputs.*/decisions.* и задан skill/feature)
      2. policy.json (новая общая конфигурация)
      3. legacy pipeline.json (для старых проектов через dual-read fallback)

    Для обратной совместимости: dotpath вида "sources.story" / "pipeline.mode" / "autonomy.criticality"
    продолжает работать (читается из legacy pipeline.json напрямую, без fallback — старые
    скрипты хуклись на конкретные ключи). НОВЫЙ код должен использовать inputs.* / decisions.* /
    pipeline.* / quality.* — config.py validate помогает мигрировать."""
    try:
        import risk_ladder as R
        # v2: используем единый config_get, который умеет dual-read в legacy.
        return R.config_get(project, dotpath, skill=skill, feature=feature)
    except Exception:  # noqa: BLE001
        return None
...
def _check_required_decisions(step: dict, project: Path, skill: str, feature: str):
    ...
    if not keys:
        return
    missing = [k for k in keys if not _config_get(project, k, skill=skill, feature=feature)]
    if not missing:
        return
    ...
    raise StepGateBlocked(
        ...
        f"manifest.json активной фичи (для inputs.*/decisions.*).\n"
        f"   Это ответ ПОЛЬЗОВАТЕЛЯ, а не догадка: спроси его (для inputs.story — «к какой "
        ...
        + "".join(f"   python3 {cfg_script} --project {project} set {k} <значение|none> --skill {skill} --feature {feature}\n"
                  for k in missing)
        + f"   ПРОВЕРЬ exit-код set: 0 = записано. exit 3 «manifest не найден» — сначала "
        f"init.py --skill {skill} --feature {feature} --steps '...', потом повтори set."
    )
...
if __name__ == "__main__":
    try:
        main()
    except StepGateBlocked as e:
        # Осознанный отказ гейта — чистое сообщение и rc=2 (как у блокирующих хуков),
        # без трейсбека: стек оставляем настоящим поломкам скрипта. См. StepGateBlocked.
        print(f"[update] DENY: {e}", file=sys.stderr)
        sys.exit(2)
