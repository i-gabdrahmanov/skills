...
дискаверит test_*.py во всех skills/*/scripts, в hooks/ (hooks/test_*.py +
hooks/tests/test_*.py) И в корне extension'а (обвязка вроде cleanup-legacy.sh),
и гоняет их текущим интерпретатором. Юнит-тесты хуков раньше не гонялись ни одним
раннером — каталог hooks/tests/ молча гнил; тесты корня — та же история.

Usage:
    python3 skills/run_all_tests.py [--skill <name>] [-q]
    python3 skills/run_all_tests.py --skill hooks   # только тесты хуков
    python3 skills/run_all_tests.py --skill root    # только тесты корня extension'а
Требует Python 3.9+ (пол пинится test_python_floor.py).
...
def _root_tests() -> list[Path]:
    """Тесты обвязки в корне репо (test_cleanup_legacy.py, test_pick_python.sh и т.п.).
    Лежат вне skills/ и hooks/, поэтому в дискавери попадают отдельной веткой.

    Тут же .sh: часть обвязки — сам shell (выбор интерпретатора в _pick_python.sh,
    установщики), и её тесты естественнее писать на bash. Раньше маска была только
    *.py, и такой тест молча не гонялся бы ни одним раннером."""
    return sorted(REPO.glob("test_*.py")) + sorted(REPO.glob("test_*.sh"))
...
def discover(skill: str | None) -> list[Path]:
    ...
    if skill:
        return skills_tests
    return skills_tests + _hooks_tests() + _root_tests()
...
def main() -> int:
    ...
    passed, failed = [], []
    for t in tests:
        # .sh гоняем bash'ем, а не sys.executable: он же и предмет теста (process
        # substitution, массивы) — sh/dash их не потянет.
        argv = ["bash", str(t)] if t.suffix == ".sh" else [sys.executable, str(t)]
