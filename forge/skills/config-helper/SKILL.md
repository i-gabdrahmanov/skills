...
**Ты (модель) НИКОГДА не редактируешь `ground/policy.json` (legacy: `pipeline.json`), `feature-gates.json` или
...
|---|---|---|
| `pipeline` | `ground/policy.json` (v2; legacy-имя `pipeline.json` всё ещё читается через dual-read для обратной совместимости, но deprecated — будет удалено в v3.0) | quality, docs, conventions, jira, bitbucket, delivery, phases_override |
...
3. **Подтверждение.** Для `sensitive`-параметров (помечены 🔒: risk-policy, decisions.auto_max_risk),
...
`set` валидирует одно значение при записи. Но скиллы читают `ground/policy.json` (с dual-read fallback на legacy `pipeline.json` для обратной совместимости) с
