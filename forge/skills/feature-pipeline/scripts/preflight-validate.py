...
def main():
    ...
    parser.add_argument("--project", default=None, help="Корень проекта")
    ...
    parser.add_argument("--skill", default=None,
                        help="Namespace (для резолвинга путей к manifest); по умолчанию — активного прогона")
    args = parser.parse_args()
    # Корень: явный --project > live-маркеры (.git/build.gradle/policy.json|pipeline.json) вверх от cwd.
    if not args.project:
        args.project = skill_paths.find_project_root()


    args.skill = _resolve_skill_ns(args.project, args.skill)
