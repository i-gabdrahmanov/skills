...
def _reset_module_cache() -> None:
    """Сбрасывает кэш модулей, которые могут быть загружены из разных layout'ов
    (hooks/ vs skills/*/scripts/) при предыдущих тестах. Без этого test_risk_ladder
    загрязняет sys.modules['_util'] / ['_project'] и test_docs_resolver_consistency
    получает чужой модуль → 14 fail'ов на 18 sub-tests."""
    for mod_name in ("_project", "_util", "_config_loader", "skill_paths",
                     "pipeline_phases", "risk_ladder", "forge_events"):
