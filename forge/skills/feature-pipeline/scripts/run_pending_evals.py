...
# Phase 0 v2 refactor: импорт _config_loader для load_project_config (двойной рид
# policy.json → pipeline.json). Скилл изолирован от hooks/_project, но hooks/ лежит
# рядом с skills/ в одном репо — дотягиваемся явно.
...
def main() -> int:
    ...
    ap.add_argument("--skill", default=None,
                    help="Namespace ground/statements/<skill>/ (по умолчанию — активного прогона)")
    ...
    args.skill = _resolve_skill_ns(root, args.skill)
    ...
    # Определяем команды. Phase 0 v2 refactor: двойной рид policy.json → pipeline.json
    # через _config_loader.load_project_config (легаси v1 продолжает работать).
    cfg = {}
    try:
