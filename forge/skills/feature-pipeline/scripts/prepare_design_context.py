...
# (легаси v1 продолжает работать). Скилл изолирован от hooks/_project, но _config_loader
# — точечный helper для ридов, дотягиваемся явно через parents[3].
...
def main():
    ...
    if not grounding_path:
        try:
            ...
            import skill_paths
            proot = Path(project_root).resolve() if project_root else skill_paths.find_project_root()
    ...
    # 2. Читаем test_layer из policy.json (legacy: pipeline.json); передаётся TDD-writer и java-writer.
    test_layer = "service-unit"
    _pcfg: dict = {}
    if pipeline_path:
        # Явный путь: --pipeline / путь к файлу. Phase 0 v2 refactor: читаем как есть.
        _pipeline_file = Path(pipeline_path)
        ...
        # Автодетект: load_project_config (двойной рид policy.json → pipeline.json).
        # Идём вверх от task-plan до корня проекта (там, где лежит ground/),
        # устойчиво к относительным/коротким путям (Qwen может передать просто
        # 'task-plan.json'); .parents[2] упал бы с IndexError.
        for _anc in Path(task_plan_path).resolve().parents:
            if (_anc / "ground").is_dir():
                try:
                    from _config_loader import load_project_config  # noqa: E402
                    _pcfg = load_project_config(_anc) or {}
                except Exception:
                    _pcfg = {}
                break
    if _pcfg:
