...
def main():
    ...
    # Если --pipeline-config задан и существует — читаем его явно (override);
    # иначе — load_project_config от корня проекта (auto-detect policy.json → pipeline.json).
    pipeline_config = None
    try:
        from _config_loader import load_project_config  # noqa: E402
    except Exception:
        load_project_config = None  # type: ignore[assignment]
    ...
    elif load_project_config is not None:
        try:
            _auto_root = Path(args.task_plan).resolve().parent
            # Поднимаемся от task-plan.json до корня проекта (там, где лежит ground/).
            for _anc in _auto_root.parents:
                if (_anc / "ground").is_dir():
                    _auto_root = _anc
                    break
            pipeline_config = load_project_config(_auto_root) or None
        except Exception:
            pipeline_config = None
