...
# Phase 0 v2 refactor: двойной рид policy.json → pipeline.json через единый helper.
# Импорт из hooks/_config_loader (skill_paths уже добавил hooks/ в sys.path через _hooks_dir).
...
def _max_iterations(project_root: Path) -> int:
    """Потолок ре-итераций судьи (policy.json/pipeline.json quality.max_judge_iterations, дефолт 3)."""
    cfg = _load_project_config(project_root) or {}
    try:
...
def _load_project_config(project_root: Path) -> dict:
    """Двойной рид policy.json → pipeline.json через hooks/_config_loader (Phase 0 v2 refactor).

    Локальная обёртка вместо прямого импорта из каждого сайта: run_judge — единая точка
    всех читателей config в этой фазе, и обёртка фиксирует контракт «пустой dict при любом
    сбое» (раньше _load_json возвращал None)."""
    try:
...
def check_red(slug: str, feature_dir: Path | None) -> dict:
    ...
    из policy.json/pipeline.json) с фильтром по тест-классам фичи и проверяет exit code."""
    import subprocess
    ...
    # Читаем test_layer + build-систему из policy.json (legacy: pipeline.json), default=service-unit/gradle.
    test_layer = "service-unit"
    build_system = "gradle"
    _pcfg = _load_project_config(project_root)
    ...
    _cfg = _load_project_config(project_root) or {}
    ...
    # Нет тест-классов фичи → гнать ВСЕ тесты модуля нельзя (несвязанный упавший тест
    # заблокировал бы фичу), но и вердикт PASS выдавать нельзя: судья не проверил НИЧЕГО.
    # Раньше здесь был WARN, и судья возвращал `passed=True` со сводкой «0/1 modules passed» —
    # фаза RED закрывалась, ни разу не запустив тесты. До этой ветки доходят только фичи с
    # задачами, которые ПИШУТ КОД и не test-exempt (иначе выше сработал бы early-return
    # `test:exempt`), поэтому пустой scope — это дефект плана, а не легальный случай.
    # Deny-first: не смог проверить — не пропускай (найдено e2e-прогоном full-ветки).
    if not feat_classes:
        detail = ("не удалось определить тест-классы фичи: в task-plan.json у задач нет "
                  "файлов из src/test/**.java в `artifacts`, и шаги 04-test-* не отдали "
                  "`test_files`. Без scope судья либо проверит ЧУЖИЕ тесты, либо не проверит "
                  "ничего — оба варианта делают вердикт бессмысленным. Впиши тест-файлы "
                  "задачи в `tasks[].artifacts` (там же, где прод-файлы) и повтори.")
        blocking_issues.append(f"red-judge scope: {detail}")
        all_passed = False
        checks.append({
            "name": "test:scope",
            "status": "FAIL",
    ...
    for label, cmd in jobs:
        try:
            ...
            # ПОЛЯРНОСТЬ RED. Судья фазы, чей смысл — «тесты обязаны падать», засчитывал
            # прогон при `returncode == 0`, то есть ровно при ЗЕЛЁНЫХ тестах. Корректное
            # RED-состояние он рубил, а состояние «RED не написан, всё зелено» — пропускал.
            # Дефект виден только на настоящей сборке: в юнит-тестах эта ветка не доходила
            # до gradle (fail-open по отсутствию feat_classes). Найдено e2e на Gradle+JaCoCo,
            # где check_tests_red говорил PASS, а red-judge на том же состоянии — FAIL.
            out = (r.stdout or "") + (r.stderr or "")
            compile_broken = ("compileTestJava FAILED" in out or "compileJava FAILED" in out
                              or "error: cannot find symbol" in out
                              or "Compilation failed" in out)
            if compile_broken:
                passed = False
                detail = "тесты фазы RED не компилируются — RED обязан компилироваться"
                blocking_issues.append(f"RED-judge {label}: {detail}")
                all_passed = False
            elif r.returncode == 0:
                passed = False
                detail = ("тесты фичи ЗЕЛЁНЫЕ (exit 0) — это не RED: тест обязан падать на "
                          "ещё не реализованном поведении")
                blocking_issues.append(f"RED-judge {label}: {detail}")
                all_passed = False
            else:
                passed = True
                detail = f"тесты фичи красные (exit {r.returncode}) — корректное RED-состояние"
...
def _check_forbidden_test_annotations(
    ...
    project_root = PROJECT_ROOT or skill_paths.find_project_root()
    for task in tp.get("tasks", []):
...
def check_spec(slug: str, feature_dir: Path | None) -> dict:
    ...
    pcfg = _load_project_config(PROJECT_ROOT) or {}
    ...
    if master_cfg.get("enabled"):
        ...
        if master_spec and master_spec.exists():
            ...
            try:
                ...
                # Не передаём --pipeline-config: гейт сам делает dual-read policy.json → pipeline.json
                # через load_project_config(find_project_root()) (Phase 0 v2 refactor). Хардкод
...
def check_sdd_doc(slug: str, feature_dir: Path | None) -> dict:
        ...
        try:
            ...
            if r.returncode == 0:
                ...
                checks.append({"name": "check_sdd_doc", "status": "FAIL",
                               "detail": detail, "severity": "error"})
                blocking_issues.append(f"check_sdd_doc FAIL: {detail}")
        except subprocess.TimeoutExpired:
            checks.append({"name": "check_sdd_doc", "status": "FAIL",
    ...
    _qual = (_load_project_config(PROJECT_ROOT) or {}).get("quality", {})
        ...
        try:
            ...
            if r.returncode == 0:
                ...
                checks.append({"name": "check_taskplan", "status": "FAIL",
                               "detail": detail, "severity": "error"})
                blocking_issues.append(f"check_taskplan FAIL: {detail}")
        except subprocess.TimeoutExpired:
            checks.append({"name": "check_taskplan", "status": "FAIL",
                           "detail": "timeout (60s)", "severity": "error"})
        ...
        try:
            ...
            if r.returncode == 0:
                ...
                checks.append({"name": "check_sdd", "status": "FAIL",
                               "detail": detail, "severity": "error"})
                blocking_issues.append(f"check_sdd FAIL: {detail}")
        except subprocess.TimeoutExpired:
            checks.append({"name": "check_sdd", "status": "FAIL",
                           "detail": "timeout (60s)", "severity": "error"})
            ...
            try:
                ...
                if r.returncode == 0:
                    ...
                    checks.append({"name": "check_adr", "status": "FAIL",
                                   "detail": detail, "severity": "error"})
                    blocking_issues.append(f"check_adr FAIL: {detail}")
            except FileNotFoundError:
...
def check_coverage(slug: str, feature_dir: Path | None) -> dict:
    ...
    # Порог из policy.json/pipeline.json quality.coverage_threshold (fallback 0.80)
    threshold = 0.80
    pipeline_cfg = _load_project_config(project_root) or {}
...
def _fail_detail(proc, limit: int = 800) -> str:
    """Причина FAIL детерминированного чекера — В ВИДЕ, ПРИГОДНОМ ДЛЯ ИСПРАВЛЕНИЯ.

    Раньше бралось `(stderr or stdout)[:200]`. Чекеры печатают JSON-отчёт, который начинается
    с абсолютного пути к проверяемому файлу, — и 200 символов уходили целиком на путь. На
    e2e-прогоне `check_sdd_doc` выдал ВОСЕМЬ конкретных ошибок («нет обязательный раздел SDD:
    «модель угроз»», «не найден ни один сценарий Given-When-Then»), а до errors.json доехало:

        check_sdd_doc FAIL: {\n  "status": "fail",\n  "sdd": "/private/tmp/.../ORD-101/

    Ни одной причины. А ведь именно errors.json скармливается субагенту на ре-итерации — то
    есть цикл исправления кормили пустышкой, и «почини» означало «угадай, что не так».

    Теперь: если чекер отдал JSON — вытаскиваем `errors` (и `warnings` как хвост), иначе
    обрезаем сырой вывод, но по более разумному лимиту."""
    ...
    if isinstance(data, dict):
        ...
        if not parts:
            parts = [str(w) for w in (data.get("warnings") or [])]
        if parts:
            joined = "; ".join(parts)
            return joined[:limit]
        if data.get("status"):
            return f"status={data['status']}"
    detail = err or raw or f"exit code {getattr(proc, 'returncode', '?')}"
    return detail[:limit]
...
def main():
    ...
    args.skill = _resolve_skill_ns(project_root, args.skill)
    ...
    if not verdict["passed"] and verdict.get("blocking_issues"):
        _ps = Path(__file__).resolve().parents[2] / "pipeline-state" / "scripts"
        _ovr = _ps / "override_judge.py"
        _RECORD_APPROVAL_SCRIPT = _ps / "record_approval.py"
        print()
        ...
        print(f"     2) после «да» зафиксируй согласие СКРИПТОМ (рукописный маркер не считается: "
              f"прямая запись в approvals/ заблокирована state-write-guard, а гейт требует "
              f"провенанс record_approval):\n"
              f"        python3 {_RECORD_APPROVAL_SCRIPT} --key gate-override-{judge_name} "
...
def _resolve_skill_ns(project, explicit):
    ...
    try:
        ...
        found = active_feature_with_skill(_Path(project))
        if found:
            return found[0]
    except Exception:
        pass
