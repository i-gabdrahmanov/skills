...
Usage:
    check_architecture.py [--root .] [--base HEAD] [--changed "a.java b.java"]
        [--pipeline-config policy.json] [--package-root ru.x.y] [--strict] [--json]
...
def main() -> int:
    ...
    if package_root is None and args.pipeline_config:
        ...
        cfg: dict = {}
        _pp = Path(args.pipeline_config)
        if _pp.exists():
            try:
                cfg = json.loads(_pp.read_text(encoding="utf-8"))
            except Exception:
                cfg = {}
        if not cfg:
            try:
    ...
    # Phase 0 v2 refactor: load_project_config — двойной рид policy.json → pipeline.json.
    # Если --pipeline-config задан и существует — читаем его; иначе fallback к load_project_config(root).
    pcfg: dict = {}
    if args.pipeline_config:
        _pp = Path(args.pipeline_config)
        if _pp.exists():
            try:
                pcfg = json.loads(_pp.read_text(encoding="utf-8"))
            except Exception:
                pcfg = {}
    if not pcfg:
        try:
