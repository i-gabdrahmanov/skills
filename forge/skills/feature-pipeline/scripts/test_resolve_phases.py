...
def _project(td: str, pipeline: dict | None) -> Path:
    """Фикстура: пишет v2-конфиг (ground/policy.json). Раньше писался pipeline.json —
    это работало пока config.py резолвил "pipeline"→pipeline.json. После миграции v2
    config.py ищет policy.json; для совместимости с тестами на phase disable/add
    (которые зовут config.py внутри) фикстура пишет policy.json. resolve_phases._load_policy
    сам умеет читать legacy pipeline.json как fallback, так что back-compat сохранён."""
    project = Path(td)
    (project / "ground").mkdir(parents=True, exist_ok=True)
    if pipeline is not None:
        (project / "ground" / "policy.json").write_text(json.dumps(pipeline), encoding="utf-8")
    return project
