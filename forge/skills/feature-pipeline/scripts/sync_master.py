...
# Phase 0 v2 refactor: импорт _config_loader для load_project_config (двойной рид policy.json
# → pipeline.json). Скилл изолирован от hooks/_project, но hooks/ лежит рядом с skills/
# в одном репо. parents[3] от этого файла — корень репо.
...
def _load_cfg(project: Path) -> dict:
    ...
    # Скилл может работать в обеих раскладках (новые v2-проекты и легаси v1).
    try:
        from _config_loader import load_project_config
        return load_project_config(project)
    except Exception:
        # Fallback (на случай битого бандла): читаем только legacy pipeline.json.
