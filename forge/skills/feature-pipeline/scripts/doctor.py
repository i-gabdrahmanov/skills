...
def run_checks(project_root: Path | None = None) -> dict:
    ...
    if sys.version_info[:2] < MIN_PYTHON:
        ...
        warn("python-version",
             f"Python {have} < {MIN_PYTHON[0]}.{MIN_PYTHON[1]} — ниже проверенного пола. "
             f"Обнови интерпретатор.")
    else:
        ok("python-version")
    ...
    # Phase 0 v2 refactor: проверяем наличие ЛЮБОГО из ground/{policy.json, pipeline.json}.
    # Новые v2-проекты — policy.json, легаси — pipeline.json; оба считаются валидным конфигом.
    config_path = None
    if project_root:
        for _name in ("policy.json", "pipeline.json"):
            _cand = project_root / "ground" / _name
            if _cand.exists():
                config_path = _cand
                break
    if not project_root:
        checks.append({"name": "config-valid", "status": "SKIP", "detail": "нет project root"})
    elif not cfg_validate.exists() or config_path is None:
        checks.append({"name": "config-valid", "status": "SKIP",
                       "detail": "нет config-helper или ground/{policy.json|pipeline.json}"})
    else:
        try:
            r = subprocess.run(
