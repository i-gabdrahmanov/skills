...
`feature-pipeline/scripts/` — только она проставляет `required_judges` и пересобирает
`gate.json`. Версию из `pipeline-state/scripts/add_steps.py` здесь НЕ применяй: она не
форсит судей build/red/delivery и не синхронизирует фазовую машину.**
...
python3 <project>/.gigacode/skills/feature-pipeline/scripts/build_evals_from_design.py \
    ...
    --coverage-script "<project>/.gigacode/tools/check_coverage.py" \
    --coverage-helper "<project>/.gigacode/tools/coverage-helper.py"
```

Скрипт генерирует для каждой задачи три типовых eval'а:
- **compile** — проверка, что проект компилируется
- **coverage** — проверка JaCoCo покрытия (инкрементальный запуск через `coverage-helper.py`, проверка через `check_coverage.py`)
- **test_pass** — проверка, что % зелёных тестов задачи >= порога
...
Запусти субагента eval-judge (промпт — §7.1 subagent-prompts.md):
```
agent(subagent_type="general-purpose", description="eval-judge for <slug>",
      prompt="<контракт eval-judge из §7.1 subagent-prompts.md + пути к task-plan.json и eval-plan.json>")
```
...
**🚨 Первое и главное правило: субагент НЕ вызывает `ask_user_question`.** 
Он только собирает черновик и возвращает JSON. Все вопросы пользователю задаёшь ТЫ.

Прочитай инструкции немедленно (ДО запуска субагента):
```
read_file("<project>/.gigacode/skills/jira-task-writer/SKILL.md")
```
Прочитай входные данные (ДО запуска субагента):
```
read_file("<папка фичи>/task-plan.json")
read_file("<папка фичи>/../brd.md")   # или business-requirements/<slug>.md
read_file("<project>/ground/pipeline.json")
```
**Только после этого** запусти субагента, передав ему все данные в одном промпте:
```
agent(subagent_type="general-purpose",
      description="Jira tasks for <slug>",
      prompt="<контракт + все прочитанные данные: task-plan.json, brd.md, pipeline.json>")
```
...
2. **Когда `pending_questions` пуст** — покажи черновик пользователю:
   `ask_user_question` «Создавать эти задачи в Jira?» с вариантами:
   - «Да» — создавай задачи через Jira MCP
   - «Правки» — уточни, что изменить
   - «Не создавать» — `skipped: true`

3. **На «Да»** — создай Story и Sub-task через Jira MCP, следуя
   `references/jira-create-workflow.md`. Сохрани результат:
   ```bash
...
4. **На пустой ответ** (не отобразился вопрос) — не паникуй:
...
Для каждой задачи вызови agent() с контрактом тестописателя из
[`references/subagent-prompts.md §4.1`](references/subagent-prompts.md#41-тестописатель-tdd--red-тесты-до-кода):
```
agent(
  subagent_type="general-purpose",
  description="TDD red: tests for <taskId> in <slug>",
  prompt="<контракт из §4.1 subagent-prompts.md; подставь: taskId, slug, acceptance, tech-design сигнатуры>"
)
```
...
agent(subagent_type="general-purpose", description="red-judge for <taskId>",
      prompt="<контракт red-judge из `<project>/.gigacode/skills/feature-pipeline/references/subagent-prompts.md` §7.2>")
```
Затем:
```bash
...
После прохождения red-judge — запусти субагента для создания стабов. Контракт — в
[`references/subagent-prompts.md §4.2`](references/subagent-prompts.md#42-стабы-сигнатур-tdd--перед-green):
```
agent(
  subagent_type="general-purpose",
  description="Stubs for <taskId> in <slug>",
  prompt="<контракт из §4.2 subagent-prompts.md; подставь: taskId, slug>"
)
```
...
agent(
  ...
  prompt="<полный контракт из references/subagent-prompts.md §4.3; подставь: taskId, slug, task-plan, tech-design>"
)
```
...
agent(subagent_type="general-purpose", description="build-judge for <slug>",
      prompt="<контракт build-judge из subagent-prompts.md §7.3>")
```
...
Контракт — в [`references/subagent-prompts.md §4.4`](references/subagent-prompts.md#44-cover-gaps--добор-покрытия):
```
agent(
  subagent_type="general-purpose",
  description="Cover gaps for <slug>",
  prompt="<контракт из §4.4 subagent-prompts.md; подставь: slug, check_coverage отчёт>"
)
```

### 8.2 Тестраннер

Контракт — в [`references/subagent-prompts.md §4.1a`](references/subagent-prompts.md#41a-тестраннер) (секция Pre-commit):
```
agent(
  subagent_type="general-purpose",
  description="Run tests + coverage for <slug>",
  prompt="<контракт из §4.1 Pre-commit subagent-prompts.md; подставь: slug>"
)
```
...
Контракт — в [`references/subagent-prompts.md §5`](references/subagent-prompts.md#5-спецадаптер):
```
agent(
  subagent_type="general-purpose",
  description="Update spec for <slug>",
  prompt="<полный контракт из §5 subagent-prompts.md; подставь: slug, docs_path, diff>"
)
```

### 9.2 Gate: enrich_grounding (детерминированно)

После спецадаптера выполни — инкрементально обновляет `docs/system-analysis/*.md`
и пересобирает `grounding-excerpt.json` по дельте фичи из task-plan.json:
```bash
...
agent(subagent_type="general-purpose", description="spec-judge for <slug>",
      prompt="<контракт spec-judge из subagent-prompts.md §7.4 + slug + docs_path + task-plan>")
```
Затем:
```bash
...
agent(subagent_type="general-purpose", description="delivery-judge for <slug>",
      prompt="<контракт delivery-judge из §7.5 subagent-prompts.md + task-plan + jira-result + git status + diff>")
```
