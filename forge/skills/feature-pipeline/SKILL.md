...
description: >
  End-to-end рабочий процесс для реализации НОВОЙ ФИЧИ от спецификации до
  верифицированного артефакта: специфицировать (SDD из идеи/Jira; бизнес-анализ/BRD
  выключен), спроектировать
  ...
  forgefix: тот — для минорного дефекта (правит спеку точечно), этот — для фичи с нуля
  (пишет спеку). Используй когда пользователь говорит "сделай фичу X", "запусти feature
  pipeline", "реализуй фичу end-to-end", "спецификацию с нуля", или описывает идею продукта
  и хочет довести её до готового кода. **Голое "forge"/"запусти forge"/"прогони forge" —
  НЕ триггер этого скилла:** это просьба прогнать задачу через харнес, а путь (fix | lite |
  full) выбирает `router` — багфикс, уехавший сюда, заведёт новую фичу и перепишет спеку
  вместо правки. Аргумент (ключ Jira вида STOR-123 либо описание фичи) — вход
  фазы спецификации (SDD). Скилл
...
Скилл ведёт фичу по циклу: **идея/Jira → контекст системы → SDD (спецификация) →
тех-дизайн → задачи в Jira → код → тесты → спека**. Бизнес-анализ/BRD выключен
(`pipeline_phases.BRD_ENABLED`) — старт с grounding + SDD. На этом пайплайн заканчивается:
...
> (`<forge>/skills/feature-pipeline/SKILL.md`). Раскладка одна:
> `<project>/.gigacode` (развёртывается через `bash deploy.sh <project>`; см. INSTALL.md).
...
- **exit 2** — конфиг не инициализирован (в JSON — `init_needed`, а **не** `errors`:
  `ground/policy.json not found`/`incomplete`). Это нормальный первый запуск.
...
- **exit 1** — посмотри `errors`. Это **ENFORCEMENT OFF**: `settings.json` / `hooks block empty` /
  `essential hook НЕ подключён` / `resolve_hook_paths` / битый
  `policy.json` — хуки реально не срабатывают. **Остановись и предупреди пользователя**:
  деплой не поднят — `bash deploy.sh <project>` (см. INSTALL.md) +
...
- MCP **Atlassian (Jira)** — **опционален**: нужен только для чтения входного issue (§2) и
  создания задач (фаза 2.5). Нет MCP → пайплайн идёт в режиме «без Jira» (issue описывает
  пользователь текстом, фаза 2.5 пропускается по `jira.enabled=false`). **Но Jira-ключ входного
  issue обязателен ВСЕГДА** — без него пайплайн не стартует (жёсткий гейт в `init.py`, оверрайда нет).
...
Фазы пайплайна определяются **не в этом SKILL.md**, а в `ground/policy.json` через скрипт
...
**Как это работает:**
- Каждая фаза имеет поле `enabled_by` — путь к булеву полю в `policy.json` или `gates.*`.
...
**Переопределение фаз:** можно добавить секцию `phases_override` в `policy.json`,
...
Конфиг разделён на два файла с разной семантикой (v2 model, см. `## v2 model`):
- **Project-wide:** `<project>/ground/policy.json` — общая конфигурация проекта
  (`quality.*`, `conventions.*`, `docs.*`, `sdd.*`, `adr.*`, `spec.*`, `jira.*`, `project.*`).
  Иммутабельна на прогоне активной фичи.
- **Per-feature:** `<project>/ground/statements/<skill>/<feature>/manifest.json` — входы и решения
  текущей фичи (`inputs.*`, `decisions.*`). Пишется ТОЛЬКО через `config.py set` с `--skill`/`--feature`.

Полная схема и правила — [`references/config.md`](references/config.md).

1. Прочитай `<project>/ground/policy.json`.
...
4. Дальше бери из конфига: `docs.*` (расположение артефактов, см. ниже),
   ...
   `project.default_branch`. Не хардкодь эти значения в шагах — читай из конфига.
   Per-feature `inputs.*`/`decisions.*` читай из manifest.json активной фичи (active_manifest).
...
1. Проверь `jira.auto_discovered` в `policy.json`. Если `true` — шаг пропускается.
...
4. Передай собранную мету в скрипт автоопределения:
   ...
   ```
5. После успешного прогона в `policy.json.jira` появятся:
...
`<project>/ground/statements/feature-pipeline/<feature>/`, где `<feature>` — **Jira-ключ
входного issue** (обязателен; тот же, что папка `docs/feature-pipeline/<key>/`). Все вызовы
pipeline-state — с `--feature <key>`. Ниже `<slug>` в командах = этот Jira-ключ; `init.py`
отвергнет любой не-ключевой `--feature` (оверрайда нет).
...
> НЕ включается в манифест, пайплайн стартует с `01-grounding` → `02-sdd`. `resolve_phases`
> кладёт `00-brd` в `skipped` — сверься с его выводом. Вернуть BRD: `BRD_ENABLED = True`,
> верни строку `00-brd` (`| \`00-brd\` | Discovery / BRD | — |`) и зависимость `02-sdd → 00-brd`.
...
### 0.6.2 Откат к шагу (rollback) — когда ре-итерация не спасает

Если проблема глубже одной фазы (гейт SDD вскрыл новое требование → переработка SDD через
мини-интервью; дизайн нежизнеспособен после провалов сборки сверх лимитов) — не крути
ре-итерации, предложи
пользователю **откат**: `pipeline-state/scripts/rollback.py --to-step <X>` («X переделывается»:
...
| ~~0 Discovery (BRD)~~ | ⛔ ОТКЛЮЧЕНА (`BRD_ENABLED=False`) — старт с фазы 1 | — | — |
...
**Jira-ключ входного issue обязателен** (`STOR-123`, формат `[A-Z]+-\d+`) — он же слаг стейта
(`--feature`). Нет ключа → **не стартуй**: попроси пользователя передать ключ (`init.py` всё равно
заблокирует создание папки стейта без валидного ключа, оверрайда нет). Подключать Jira MCP не
обязательно — только передать ключ.

Определи вход (бизнес-анализ/BRD выключен — идём прямо в фазу спецификации SDD, фазы 0 нет):
- **MCP Atlassian есть** → прочитай issue по ключу через MCP, передай summary/description +
  ключ как вход SDD-писателю.
- **MCP нет** («без Jira») → возьми описание фичи из свободного текста пользователя, ключ используй
  как идентификатор/слаг; неясности SDD-писатель добирает мини-интервью (`pending_questions`).
...
|---|---|---|
| 00-brd | `references/phases/00-brd.md` | ⛔ ОТКЛЮЧЕНА (`BRD_ENABLED=False`) — `resolve_phases` пропускает |
| 01-grounding | `references/phases/01-grounding.md` | grounding-excerpt готов |
| 02-sdd | `references/phases/02-sdd.md` | check_sdd_doc + Гейт SDD + гейт критичности |
...
**Защита от пустого ответа ask_user_question (fallback = STOP, НЕ skip).** Если
`ask_user_question` вернул пустой ответ (пользователь не ответил / ответ не доставлен, напр.
headless `-p` или форк не рендерит диалог) — повтори вопрос **не более 1 раза**. Второй пустой
ответ подряд означает, что вопрос не доставляется. В этом случае:
1. **Остановись** (не пропускай фазу молча).
2. Напиши пользователю текстовое сообщение: «Я задал вопрос, но не получил ответ.
   Ответь на вопрос выше, либо предзапиши решение в конфиг и перезапусти:
   `config.py set <ключ> <значение>` (напр. `decisions.criticality`, `inputs.spec`, `inputs.mode`).»
3. Не вызывай `ask_user_question` в третий раз. **НЕ пропускай обязательный шаг** — `update.py`
   заблокирует `skipped` для обязательной фазы (REQUIRED_STEP_PREFIXES) с `exit 3 ESCALATE`; это и
   есть fail-closed. Пропуск возможен ТОЛЬКО для необязательных шагов или через
   `override_judge --judge step-skip-<step_id>` после явного «да» пользователя.

> **Универсальный режим решений (headless и интерактив).** Каждое решение (критичность, путь,
> спека) — обязательный артефакт: per-feature (`inputs.*`/`decisions.*`) живёт в
> `manifest.json` активной фичи, project-wide (`quality.*`/`conventions.*`/…) — в `policy.json`.
> Интерактивно его заполняет ответ на вопрос;
> в headless — предзапись `config.py set …` ДО прогона. Нет артефакта → `gate-guard` блокирует
> продуктивную запись фазы (`required_decisions` в risk-policy.json). Вопрос — удобный способ
> заполнить артефакт, а не единственный: пайплайн не зависит от того, отрендерился ли диалог.
...
- **Project-wide (`ground/policy.json`)** — общая конфигурация проекта: `quality.*`,
  ...
  `phases_override`. Иммутабельна на прогоне активной фичи (манифест существует) — `set`
  вернёт exit 1, `phase` тоже.
- **Per-feature (`ground/statements/feature-pipeline/<slug>/manifest.json`)** — входы и решения
  текущей фичи: `inputs.mode`/`inputs.spec` и `decisions.mode_task`/
...
- **`risk-policy.json::required_decisions` / `required_decisions_on_close`** — пути
  ключей теперь идут в формате `inputs.story`, `inputs.spec` и т.п. Старые пути
  `sources.*` в risk-policy работают через dual-read и НЕ требуют миграции (deprecated
  flag на реестре, не на списке решений).

Пример (full-путь, запись `decisions.criticality` для активной фичи):
```
python3 <project>/.gigacode/skills/config-helper/scripts/config.py --project <toplevel> \
  set decisions.criticality high --skill feature-pipeline --feature STOR-123
```
...
```
Пример (выключить фазу 02-eval-plan — пишет в `policy.json.phases_override`,
на прогоне активной фичи вернёт exit 1):
```
python3 <project>/.gigacode/skills/config-helper/scripts/config.py --project <toplevel> \
  phase disable 02-eval-plan
```
