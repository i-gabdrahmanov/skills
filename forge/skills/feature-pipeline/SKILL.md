...
Определи вход (бизнес-анализ/BRD выключен — обе ветки идут прямо в фазу спецификации SDD,
фазы 0 нет):
- **Свободная идея** → передай её как исходную идею SDD-писателю (§4.0a); неясности он
  добирает мини-интервью (`pending_questions`).
- **Ключ Jira** (`STOR-123`, формат `[A-Z]+-\d+`) → прочитай issue через MCP, передай
  summary/description + ключ как вход SDD-писателю.
