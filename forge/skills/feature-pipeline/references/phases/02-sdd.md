...
Если ведётся требования-мастер (`docs.master.enabled`) — добавь в промпт путь
`ground/inventory/spec-conventions.json` (профиль формы мастера, снят на 01-grounding). Формат
самой дельты при этом НЕ меняется — `sdd.md` всегда по форже-шаблону; важны **названия**
требований: `/forge-merge` сопоставляет дельту с мастером по нормализованному названию, и своя
формулировка вместо принятой в мастере даст ложное «новое требование» вместо «изменено».
