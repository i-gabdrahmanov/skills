...
Если `status` пишет «ФОРМАТ МАСТЕРА НЕ РАЗОБРАН» — состояние дельты относительно мастера
НЕИЗВЕСТНО (и архивация её не отпустит). Не выдавай это за «слито» и не предлагай merge:
скажи пользователю выполнить `/forge-spec research` — снять профиль формата его спеки.
