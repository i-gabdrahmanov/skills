...
>
> **Гейт закрытия фазы:** `check_brd_doc.py` PASS (детерминированный хард-гейт) + Гейт
> доставки BRD (мердж на согласование — опционально, с ПАУЗОЙ) + Гейт 1 (пользователь
> утвердил BRD; маркер `brd-approved-<slug>` — enforced в update.py) + brd-judge advisory
> + гейт критичности; закрой шаг 00-brd
...
### Гейт доставки BRD — «нужен мердж и пуш?» (ДО утверждения)

BRD, прошедший brd-judge, можно вынести на согласование аналитикам коммитом `brd.md` на
**ветку задачи `docs/<slug>`**: у каждой Jira-задачи — своя ветка с её доками (сюда же в
фазе 02 приедет `sdd.md`), база — default-ветка (в `docs.mode=separate-repo` — репо спеки;
PR не создаётся).

1. Проверь предзаписанное решение: `config.py get docs.brd_review`.
   `push` → сразу к шагу 3; `skip` → сразу Гейт 1 (ниже); пусто → спроси (шаг 2).
2. Спроси через `ask_user_question` (header: «Мердж BRD», 2 опции):
   «Нужен мердж и пуш `brd.md` на ветку задачи `docs/<slug>` для согласования с
   аналитиками? Коммитится только brd.md; после пуша пайплайн берёт паузу до итогов ревью.»
   - «Да» — коммит на ветку задачи + пуш + ПАУЗА (шаг 3); «Нет» — сразу Гейт 1.
   Ответ зафиксируй: `config.py set docs.brd_review push` (или `skip`). Пустой ответ —
   правило SKILL.md §0.7 (fallback = STOP + предзапись), гейт не пропускается молча.
3. При «да» — согласие СКРИПТОМ (прямая запись в approvals/ заблокирована state-write-guard),
   затем санкционированная доставка (сырые `git commit`/`git push` — НЕ пытайся руками):
   ```bash
   python3 <project>/.gigacode/skills/pipeline-state/scripts/record_approval.py \
       --project <project> --key brd-review-<slug> --approved-by user \
       --reason "пользователь согласовал доставку BRD на ветку задачи аналитикам"
   python3 <project>/.gigacode/skills/feature-pipeline/scripts/doc_review_push.py \
       --doc brd --feature <slug> --jira-key <KEY> --json
   ```
   Скрипт проверит маркер и PASS brd-judge, secret-scan, закоммитит ТОЛЬКО `brd.md` поверх
   remote-tip ветки `docs/<slug>` (нет ветки — создаст от default-ветки) и запушит без
   force; worktree/локальные ветки не тронет (идемпотентен; состояние — `--status`).
   - exit 0 → **ПАУЗА**: сообщи пользователю ветку задачи/коммит и что пайплайн стоит до
     итогов ревью, и ЗАВЕРШИ ход. Шаг `00-brd` остаётся `in_progress` — update.py не даст
     закрыть его без утверждения (enforced, см. Гейт 1). НЕ спрашивай утверждение тем же ходом.
   - exit ≠ 0 → покажи stderr пользователю и спроси, как поступить.

   **Резюме после паузы:** правки аналитиков → верни BRD-писателя на доработку (после
   доработки — заново `check_brd_doc`/brd-judge и повтори доставку, шаг 3); «согласовано» →
   Гейт 1 (ниже).
...
python3 <project>/.gigacode/skills/pipeline-state/scripts/record_approval.py \
    ...
    --reason "BRD утверждён пользователем<; аналитики согласовали — если был мердж>"
```
Обнови `00-brd` (`gates: {"brd-judge": "PASS"}`; `brd_review_branch`/`brd_review_commit`
в artifacts — только если была доставка на согласование).
