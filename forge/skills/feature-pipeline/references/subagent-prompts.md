...
## 4.0b Ресерчер формата спеки (фаза 01-grounding, только по exit 2 детектора)

Зови ТОЛЬКО когда `analyze_spec.py` вернул exit 2 — форма мастера не распознана уверенно
регулярками. Задача субагента — прочитать мастер и назвать его форму в терминах профиля;
писать в мастер и в policy.json он НЕ имеет права (policy правит человек через `config.py set`,
детект — скрипт через `--apply-research`).

```
description: "Research master spec format"
subagent_type: general-purpose

prompt:
Ты — аналитик формата спецификации. Задача: описать, КАК устроен требования-мастер этого
проекта, чтобы движок слияния умел его читать и дописывать.

Вход:
- Мастер-спека: <путь к файлу>
- Черновой детект: <путь к ground/inventory/spec-conventions.json>

Шаг 1: Прочитай мастер (при необходимости соседние файлы спеки) и черновой детект.
Шаг 2: Ответь на пять вопросов, опираясь на ЦИТАТЫ из файла, а не на общие соображения:
  1. Чем в этом документе является ОДНО требование (какой заголовок/строка его открывает,
     какого уровня, есть ли стабильный ID и какой у него вид)?
  2. Как записаны приёмочные сценарии (строка Given-When-Then, подзаголовок «Scenario:»,
     список, не ведутся вовсе)?
  3. В каком разделе лежат требования и есть ли раздел журнала изменений?
  4. Помечается ли происхождение требования (тег вида [from: …])?
  5. Что в этом формате НЕ выражается полями профиля ниже (если такое есть)?

НИЧЕГО не правь: ни мастер, ни конфиг. Ты возвращаешь только JSON.

Выходной JSON (ТОЛЬКО его):
  {"grammar": {
     "requirement": {"level": 2, "kind": "id-colon|title-only|numbered|bullet-id",
                     "lead": "<слово перед названием или пусто>",
                     "id_prefix": "REQ", "id_width": 4,
                     "scope": "document|section"},
     "requirements_section": ["<кусок заголовка раздела требований>"],
     "audit_section": ["<кусок заголовка журнала изменений>"],
     "scenario": {"style": "gwt-inline|gwt-block|bullet|none", "level": 4},
     "provenance": "from-bracket|none"},
   "spec_path": "<путь мастера относительно базы, {capability} вместо имени капабилити>",
   "confidence": 0.0-1.0,
   "evidence": ["<цитата из мастера>", "..."],
   "not_expressible": ["<что профилем не описывается, если есть>"]}
```

Оркестратор применяет ответ скриптом (модель в файл профиля не пишет):
```bash
python3 <project>/.gigacode/skills/system-analyst/scripts/analyze_spec.py \
    --root "<project>" --apply-research <файл.json>
```
и показывает пользователю `/forge-spec research` — там же перечислены команды
(`config.py set` по ключам секции `spec.grammar`), которыми профиль закрепляется в policy.json. Если субагент вернул непустой
`not_expressible` — скажи об этом пользователю прямо: merge в таком формате будет отказывать
(exit 3), пока форма не сведётся к полям профиля.

## 4.0a SDD-писатель (фаза 02-sdd — идея/Jira → sdd.md)

Зови **после сбора grounding** (это первая содержательная фаза: бизнес-анализ/BRD выключен —
см. `pipeline_phases.BRD_ENABLED`), ДО tech-design. Этот субагент пишет строгую спецификацию
(`sdd.md`, PDLC v3.5) из **исходной идеи/Jira-issue** (а если BRD включён и передан — из BRD):
что система должна делать в проверяемой форме (Given-When-Then, API-контракты, модель данных,
критерии приёмки). Он НЕ раскладывает по слоям и НЕ дробит на задачи — это работа tech-design
(фаза 02-design).
...
Вход:
- Исходная идея/описание фичи: <текст: ЧТО нужно сделать и зачем>
- BRD (если бизнес-анализ включён и передан): <путь к brd.md или «нет»>
- Grounding (выжимка о системе): <путь к grounding-excerpt.json>
- Формат требований-мастера (если мастер ведётся): <путь к ground/inventory/spec-conventions.json
  или «нет»> — СПРАВКА о том, как называются и формулируются требования в мастере проекта.
  Формат самого sdd.md не меняется (всегда шаблон sdd), но НАЗВАНИЯ требований бери в той же
  форме: слияние дельты с мастером идёт по совпадению названия.
- Jira-ключ (если есть): <KEY-123 или «нет»>
- answers (если перезапуск): <ответы на прошлые pending_questions или «нет»>

Шаг 1: Прочитай исходную идею/Jira (и BRD, если передан) и grounding. Специфицируй в первую
  очередь ПО grounding; но ты в режиме дискавери — читать исходники РАЗРЕШЕНО, точечно
  свериться/закрыть пробел, когда grounding не хватает (не сканируй репо целиком).
Шаг 2: **Собери все существенные неясности** (всё, что иначе угадывал бы: развилки
  поведения, лимиты/дефолты/ошибки, переиспользовать или заводить новое, пробелы в
  идее/требованиях). Без BRD мини-интервью — ОСНОВНОЙ способ добрать требования, не ленись.
  Учти уже пришедшие `answers` и не переспрашивай закрытое.
...
3. Проверь, что docs/feature-pipeline/ не замусорен:
   ...
   b. Только текущая фича. Доки завершённых фич уезжают в <docs_base>/archive/ — сам архив
      замусоренностью НЕ считается и в этой проверке не участвует (он сиблинг
      feature-pipeline/, а не папка внутри неё). Папка прошлой фичи лежит рядом с текущей —
      это и есть замусоренность: её убирают `/forge-merge` или `/forge-archive put <слаг>`.
...
5. **Правописание и язык.** Прочитай прозу `tech-design.md` и `sdd.md` и проверь русский текст
   ...
   склейки) и опечатки. Для каждой находки — цитата и исправление; клади в `warnings` (или
   `blocking_issues`, если смысл искажён). Китайские/CJK-символы уже блокирует детерминированный
   `run_judge.py spec`/`sdd` — их НЕ дублируй, это только про валидную кириллицу.
...
6. **Правописание и язык.** Проверь русский текст на орфографические ошибки, «странные»/
   несуществующие слова (машинный перевод, обрезки, склейки, подмена букв) и опечатки. Для
   каждой находки — цитата и исправление. (Китайские/CJK-символы уже блокирует
   детерминированный `run_judge.py brd` — их НЕ дублируй, это только про валидную кириллицу.)
...
- Текст нечитаем из-за орфографии / «странных» слов — смысл требования искажён

Критерии WARN (не блокируют, но отметь):
- Пограничные CamelCase-термины, которые могут быть названиями систем/продуктов
- Орфографические ошибки / опечатки / «странные» слова, если смысл ещё читается
...
Верни JSON:
{
  ...
  "checks": [
    ...
    {"name": "Правописание/язык", "status": "WARN", "detail": "опечатка 'уведамление' → 'уведомление'", "severity": "warning"}
  ],
  ...
  "warnings": ["Термин 'TaskFlow' — проверь, это название системы или класса", "Опечатка: 'уведамление' → 'уведомление' (§Цели)"],
