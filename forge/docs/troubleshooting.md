...
**Действие:** не ведите пайплайн вслепую. Перепроверьте установку (user-guide §3) → `preflight.py` снова.
...
### B6. `[archive] DENY: ...` — стройка не уезжает в архив
**Причина** (`exit 2`) — одна из трёх: прогон не завершён (перечислены незакрытые шаги);
внутри архивируемого каталога идёт чужой незавершённый прогон (типично: архивируете стори, а в
её `fixes/` живой баг); дельта не сведена с мастером — архив уносит `sdd.md` из обхода
`/forge-spec`, и требование пропало бы молча.
**Фикс:** закройте прогон / дождитесь вложенного / `/forge-merge <слаг>`. Осознанный обход —
`--force --reason '<почему>'`, причина пишется в `archive-meta.json`.
`exit 3` — не отказ, а «нужно решение»: слаг неоднозначен (короткий ключ бага есть в нескольких
стори) либо фича заведена в нескольких namespace — назовите полный слаг или `--skill`.

### B7. `state-write-guard`: запись в `ground/archive/`
**Причина:** разложить стейт по архиву руками (`mv ground/statements/... ground/archive/...`).
Это control-plane: те же манифест, evidence и вердикты, что в `statements/`.
**Фикс:** `/forge-archive put <слаг>` — он держит гейты готовности и уносит доки со стейтом
одной операцией. Ручной `mv` разводит половины: стейт в архиве, доки на рабочем столе.
