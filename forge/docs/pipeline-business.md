...
готового, проверенного кода. Он сам выполняет рутину между контрольными точками
(собрать требования, изучить систему, спроектировать, написать код по тестам, прогнать покрытие,
обновить документацию), но **ничего необратимого не делает без явного «да» человека**.

**Доставка — за человеком.** Ветку, коммит, push, pull request и отчёт в Jira конвейер **не
делает**: он заканчивается тем, что код написан, тесты зелёные, покрытие набрано и спецификация
обновлена. Дальше разработчик распоряжается результатом сам. Единственное необратимое действие
внутри конвейера — создание задач в Jira, и оно тоже только после подтверждения.
...
## 2. Маршрут фичи: от идеи до готового кода
...
flowchart LR
    ...
    G --> H[Документация]
    H --> I[/Готовый артефакт:<br/>код · тесты · спека/]
    I -.-> D([Доставка руками:<br/>коммит · PR · отчёт])

    classDef gate fill:#fff3d6,stroke:#d9a441,color:#5b4300;
    classDef auto fill:#e6f0ff,stroke:#6ea8fe,color:#0b3a7a;
    classDef manual fill:#eeeeee,stroke:#999999,color:#333333;
    class B,C,S,E gate;
    class F,G,H,I auto;
    class D manual;
```
...
⬜ серое — **вне конвейера**, делает разработчик.
Жёлтых точек больше одной на узел там, где их несколько: «Спецификация» = подтверждение
SDD и дизайна. Плюс сразу после BRD — **выбор критичности**. Автономно (без участия человека)
идут Код, Покрытие и Документация.
...
| **9. Доставка** | **Вне конвейера.** Ветку, коммит, PR и отчёт делает разработчик | Контроль над тем, что уходит в репозиторий, остаётся у человека |
...
**Гейты 1–3** — ключевые точки; плюс именованные подтверждения (критичность, grounding,
...
> Гейт 3 (создание задач в Jira) — единственное необратимое действие внутри конвейера.
> Коммит, PR и отчёт гейтов не имеют, потому что конвейер их не делает вовсе.
...
Ветки, PR и отчёт в Jira в таблице нет намеренно: конвейер их не делает, время на них не меняется.
...
- На каждом шаге — **автоматический судья**: требования написаны языком бизнеса, тесты реально
  падают до кода, покрытие достигнуто, документация обновлена.
...
- **Это для новых фич.** Для минорного бага есть отдельный, более лёгкий маршрут — `/forge-fix`.
- Конвейер **не заменяет ревью** — он готовит качественный код к ревью человеком.
- **Доставка не автоматизирована** — по решению: код в репозиторий отправляет человек.
