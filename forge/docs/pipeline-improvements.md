...
> ⚠️ **Исторический документ.** Часть находок относится к доставке (`07-deliver`, `delivery_plan.py`,
> `check_delivery.py`, хук `evidence-enforcer`, stacked-PR) — этого слоя в форже **больше нет**:
> коммит, push, PR и отчёт делает пользователь сам (BR-13). Такие пункты читать как летопись
> решений, а не как описание текущего поведения. Актуальный состав фаз — `resolve_phases.py`,
> актуальный ростер хуков — `hooks/DEPLOY.md`.
