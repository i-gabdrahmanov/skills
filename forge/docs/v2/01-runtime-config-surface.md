...
- **`agent_type` есть ТОЛЬКО на `SubagentStart`/`SubagentStop`** (= `general-purpose`,
  плюс `agent_id` вида `general-purpose-986521589`). В `PreToolUse`/`PostToolUse` полей
  `agent_type`/`agent_id` НЕТ — ни у оркестратора, ни у субагента; `session_id` и
  `transcript_path` у них общие. **Замерено e2e на qwen-code 0.21.14** (метапроект,
  хук-рекордер на всех событиях), обе прежние формулировки в репо были неверны.
  Следствия: SoD по `agent_type` в `gate-guard` неактивен по построению; актора для
  `inline-phase-guard` даёт отметка сессии `hooks/subagent_scope.py`, которую ставит
  `context-injector` (SubagentStart) и снимает `state-recorder` (SubagentStop) — см. tasks/008.
- **`stop_hook_active` на `Stop` всегда `true`**, включая первый Stop свежей сессии
  (на `SubagentStop` — `false`). Как защита от петли поле бесполезно: `phase-gate` его не
  читает и держит одноразовость своим маркером сессии (tasks/012).
- **Инструмент `agent` в headless-режиме требует `-y`/YOLO**: без него
  `Tool "agent" requires user approval but cannot execute in non-interactive mode`, и модель
  уходит делать работу фазы сама — прямо в тот блок, который ставит `inline-phase-guard`.
- **Хуки в 0.21.14 включены нативно**, флаг `--experimental-hooks` не нужен; есть команда
  `qwen hooks` и `--safe-mode` (отключает хуки/скиллы целиком — удобно для контрольного прогона).
