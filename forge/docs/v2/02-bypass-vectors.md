...
- **Токен-бюджет осознанно НЕ тормоз** (`budget-meter`, бывший `cost-breaker`). Circuit-breaker
  удалён — хук только считает; учёт расходится с реальностью ~60× (budget.json 42%/843K при реальных
  51.8M input-токенов).
