...
# Именно этот интерпретатор resolve_hook_paths.py запечёт в settings.json как ${PYTHON}
# (через sys.executable) для КАЖДОГО хука. Поэтому берём не «первый из PATH», а первый
# здоровый: версия >= пола и живой stdlib (expat). Старый/битый python здесь означает
# хуки, падающие на импорте, — а крэш хука рантайм читает как «возражений нет».
# См. hooks/_pick_python.sh.
# shellcheck source=hooks/_pick_python.sh
. "$DIR/hooks/_pick_python.sh"
if ! forge_pick_python && [ "${#PY[@]}" -eq 0 ]; then
  echo "deploy-local.sh: не найден ни python3, ни python, ни py в PATH." >&2
  echo "  Поставь Python 3.9+ и убедись, что он добавлен в PATH." >&2
  exit 1
fi
