...
`structure-mapper` (синхронно, находит модули) → **10 субагентов параллельно**
(7 коллекторов + 3 синтезатора: use-cases, glossary, operations) → главный
агент собирает MD из JSON-ответов.
...
- В `~/.gigacode/skills/minor-defect-fix/config.json` есть `docs_path` для текущего
...
**Конвенция state'а:**
`<project>/.gigacode/statements/system-analysis/pipeline/`
...
python ~/.gigacode/skills/pipeline-state/scripts/init.py \
    ...
    --steps '[
        ...
        {"id": "02-api",            "title": "Map REST API",         "depends_on": ["01-structure"]},
    ...
    --context "$(echo '{}' | jq --arg b "$BUSINESS_INTENT" --arg s "$SCOPE" '. + {business_intent:$b, scope:$s}')"
```
...
## 3. Этап 2: субагенты параллельно

После structure-mapper вызови запрошенные субагенты **параллельно**. Каждый
получает: путь к корню, список модулей, скоуп.
...
Главный агент берёт все JSON-ответы и пишет в `<docs_path>/system-analysis/`:
...
2. Сводка цифрами:
   > "Просканировано 3 модуля. Найдено: 24 REST endpoint, 8 Kafka listeners,
   > 18 entities, 12 таблиц в схеме БД, 4 внешних клиента."
3. Спроси:
...
- `references/subagents.md` — полные промпты для 7 субагентов этапа 2.
