...
- [x] **Thrust 7 — разбор прогонов fix/lite (2026-08-12): гард глушил сам пайплайн, решения и
      дизайн терялись.** Пять симптомов одного прогона, у четырёх — код-причина.
      (1) **`state-write-guard` блокировал запись pipeline-state.** Bash-ветка блокировала по
      «в команде есть токен записи И упомянут путь», поэтому `2>&1`, `>/dev/null` или `>` внутри
      кавычек делали САМ ВЫЗОВ `update.py`/`config.py`/`run_judge.py` «записью в каталог харнеса»
      (путь скрипта = токен внутри харнеса) → deny на санкционированном пути. Теперь из команды
      извлекаются реальные ЦЕЛИ записи (`_write_targets`: редиректы, `tee`, `dd of=`, `sed -i`,
      `cp`/`mv`, `truncate`, литералы inline-python) и проверяются только они; попутно закрыты
      `dd`/`truncate`/`>|` как векторы и снят ложный deny на `cat <control-plane> … > /tmp/x`.
      Относительные пути Write резолвятся от cwd СЕССИИ, а не процесса-хука.
      ⚠️ **Цель редиректа обязана исключаться из argv команды.** Первая версия извлекателя этого
      не делала, и в `cp src <control-plane> > /dev/null` последним аргументом `cp` оказывался
      `/dev/null` — настоящее назначение копии не проверялось вовсе. Пин:
      `test_block_copy_with_trailing_redirect`.
      (2) **Ответ пользователя терялся молча.** На свежем проекте нет `ground/pipeline.json`, а
      брифы fix/lite/router знали только exit 0/1 preflight — exit 2 («не инициализирован») не
      обрабатывался, и каждый `config.py set` падал exit 3 незамеченным. Теперь preflight печатает
      готовый `init_command`, брифы обрабатывают exit 2, а `update.py` не закрывает `fix-intake`
      с незаписанным `sources.story` (новый класс `required_decisions_on_close` в risk-policy:
      решение форсится В ТОЧКЕ ВОПРОСА, а не при записи следующей фазы).
      (3) **Имя артефакта не было контрактом.** lite-документ уехал под именем слага задачи —
      гейты фазы этого не видят (путь им передаёт сама модель), а для `lite-red`/`lite-green` и
      для `/forge-merge` (ищет строго `sdd.md`) такого файла просто нет. `update.py` теперь не
      закрывает `lite-design`/`fix-diag`/`fix-spec` без канонических имён в резолвнутом docs-каталоге
      (escape: override `artifacts-<step>`).
      (4) **Утверждённый дизайн не доезжал до исполнителя.** Промпты `lite-red/green` и
      `fix-red/green` не содержали ПУТЕЙ к `tech-design.md`/`task-plan.json`/`fix-plan.md` — только
      пересказ оркестратора, а субагент конфиг не читает. Теперь путь вкладывается явно + правило
      «расходишься с планом — `status:"failed"`, а не молча по-своему».
      (5) **Команда мерджа была громоздкой:** печатался полный слаг `<стори>/fixes/<баг>`, хотя
      `merge`/`diff` принимают короткий ключ бага. `spec_cli` подсказывает короткую форму
      (`/forge-merge BUG-512 --allow-modify`), разворачивая её в полную только при коллизии.
      Пины: `test_state-write-guard.py` (`TSanctionedScriptsNotBlocked`), `test_update.py`
      (решения на закрытии + канонические имена), `test_fix_cycle.py`, `test_gate_result.py`.

- [x] **Thrust 8 — сквозной аудит пайплайна (2026-08-12): «гейт есть, но не срабатывает».**
      Один общий корень у большинства находок: **резолвер активной фазы**. `risk_ladder` ещё в
      Thrust 6 задокументировал, что статус `in_progress` на живых прогонах не проставляет никто
      (`update.py` ведёт шаг pending → completed), и `gate-guard` перевели на `current_step_id` —
      но остальные читатели остались на `in_progress`. Проверено пробами на РЕАЛЬНОМ манифесте
      (тесты этого не ловили: они выставляют `in_progress` руками).
      (1) **`inline-phase-guard` был мёртв целиком** — фаза не резолвилась → fail-open, и
      оркестратор мог писать sdd.md/tech-design.md/код фазы inline. Без SubagentStop нет
      origin-evidence, шаг потом не закрывается, и «выход» ведёт в R4-override. Он смотрит НЕ
      один шаг, а все готовые (`risk_ladder.ready_step_ids`): на параллельных задачах
      `current_step_id` намеренно отдаёт None, и одного шага хватило бы, чтобы хук снова молчал
      ровно на build-фазе. Для ролевого SoD так делать нельзя (роли фаз конфликтуют) — там
      неоднозначность остаётся fail-open.
      (2) **`sod-enforcer` молчал всегда** по той же причине (числился активным enforcement'ом).
      Блок-сообщение теперь ещё и говорит, как поступить легально (переоткрыть шаг реализации —
      это считает `max_step_reopens`).
      (3) **`tdd-guard`: привязка к задаче не работала** → он падал в легаси-фолбэк «любой
      pending test-шаг блокирует src/main», т.е. код T1 блокировался незакрытым RED задачи T2 —
      ровно тот ложный блок, который привязка и снимала. Владелец файла определяется по
      `artifacts` task-plan (новый единый предикат `pipeline_phases.task_of_artifact`).
      (4) **`eval-guard` не срабатывал по ДВУМ причинам:** слаг фичи читался только из
      `manifest.context.feature` (а `--context` у `init.py` опционален, дефолт `{}`), и задача
      резолвилась по `in_progress`. EDD-гейта фактически не существовало.
      (5) **`skill_paths` отдавал несуществующие пути в extension-раскладке**: реестр записан в
      терминах legacy (`.gigacode/skills/...`) и приклеивался к корню ПРОЕКТА, где этого каталога
      нет. `run_judge` запускал по такому пути гейты, и sdd/design/coverage-судьи валились
      «can't open file» — то есть full-путь блокировался на первом же судье, а сообщение судьи
      предлагало снять гейт override'ом. Резолв стал двухшаговым (проект → бандл), реестр бандла
      подхватывается. Тот же класс — литерал `.gigacode/...` в подсказке `eval-guard`.
      (6) **`config.py set sources.story none` писал `null`** — а `none` это ОСОЗНАННЫЙ ответ
      («стори неизвестна»), который гейты решений отличают от «не ответили» по непустому
      значению. Документированный ответ давал ДЕДЛОК: config.py отвечал `applied`, фаза
      продолжала требовать то же решение. Флаг реестра `none_is_value`.
      (7) **`state-recorder` выбирал «финальный JSON» субагента ПО ДЛИНЕ** — любой более длинный
      JSON в ответе (вывод record_gate, отчёт покрытия) вытеснял контрактный, `step_id` терялся,
      и хук молча не делал ничего: ни origin, ни закрытия шага. Теперь — последний объект
      верхнего уровня с `step_id`.
      Пины: `test_inline-phase-guard.py`/`test_sod-enforcer.py` (`RealManifestNoInProgress`),
      `test_tdd-guard.py` (`PerTaskBindingWithoutInProgress`), `test_eval-guard.py`,
      `test_skill_paths_loader.py` (`ExtensionLayout`), `test_config.py`, `test_state-recorder.py`.

- [x] **Thrust 9 — три bypass-вектора из аудита закрыты (2026-08-12).**
      (1) **Подделка СУБСТАНЦИИ гейта.** `record_gate --cmd "true"` писал валидный `passed:true`
      с правильным провенансом и закрывал любой шаг: цепочка проверяла, КТО записал evidence, но
      не ЧТО прогнали. Новый класс политики `gate_cmd_expect` (префикс шага → допустимые токены
      команды, собраны по фактическим командам брифов): нет гейта фазы в `--cmd`/`--compile-cmd`
      → отказ ДО запуска, evidence не пишется. Нет записи для шага → проверка не применяется.
      (2) **`update.py --skip-judges` был выключателем enforcement'а в одну опцию** (снимает
      судей, gate-result, subagent-origin, решения, артефакты) и был описан в брифе как
      служебный. Теперь R4 по образцу rollback: approval-маркер `skip-judges-<feature>` с
      провенансом `record_approval`, два слоя (gate-guard блокирует команду, update.py
      валидирует маркер сам — держится и мимо харнеса).
      (3) **Эскейп «задача без кода → без RED» был невыполним.** Брифы lite/fix предлагали «не
      заводи шаг», но манифест этих ветвей статический (`init.py` берёт весь список из
      `manifest-steps.json`), а `skipped` блокировался как для обязательного шага: шаг висел в
      `pending` до конца прогона либо снимался override'ом. Теперь `update.py` разрешает пропуск
      RED-шага ДЕТЕРМИНИРОВАННО — по task-plan тем же предикатом, что у `tdd-guard`
      (`all_tasks_test_exempt`/`task_is_test_exempt`); задача пишет код или плана нет →
      fail-closed. Брифы переписаны на реальный механизм.
      Пины: `test_gate_result.py` (`test_record_gate_rejects_command_without_the_gate`),
      `test_gate-guard.py` (`TSkipJudges`), `test_update.py` (skip-judges + RED-skip).
