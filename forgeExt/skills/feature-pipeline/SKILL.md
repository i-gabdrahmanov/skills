...
2. Напиши пользователю текстовое сообщение: «Я задал вопрос, но не получил ответ.
   ...
   `config.py set <ключ> <значение>` (напр. `autonomy.criticality`, `sources.spec`, `pipeline.mode`).»
...
> спека) — обязательный артефакт в `pipeline.json`. Интерактивно его заполняет ответ на вопрос;
