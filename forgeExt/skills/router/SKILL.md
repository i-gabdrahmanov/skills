...
description: >
  ...
  из jira», «прогони KIDPPRB-1234», «нужно реализовать <фичу/задачу>», «запусти forge».
...
>   (коммиты/PR — на пользователе). Путь `feature-pipeline`.
...
>   (коммиты/PR — на пользователе). Путь `forgelite`.
...
- Свободная идея, Story/Epic, нет AC/нет спеки, несколько сценариев, спека пишется с нуля → **full**.
- Просто назвали «feature pipeline / forge» без «с нуля/без спеки» — это НЕ сигнал full; классифицируй по сути.
