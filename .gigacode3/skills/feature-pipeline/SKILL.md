...
> **Control plane на хуках — Forge PDLC v3.5 (gigacode3).** Enforcement форсится рантайм-хуками
> (`~/.gigacode/hooks/`, см. `hooks/DEPLOY.md`), а не текстом — *hooks = enforcement, SKILL.md = guidance*:
> - **Risk ladder R0–R5 / deny-first** (`gate-guard`+`risk-policy.json`): действие классифицируется по
>   риску/blast-radius; рисковое (R3+) блокируется, пока не выполнено требование уровня (шаги manifest /
>   approval-маркер `ground/approvals/<key>.json` / evidence). Separation of duties: роль-субагент не может
>   действие выше своего cap.
> - **Evidence bundle** (`evidence-enforcer` + `build_evidence.py`/`check_evidence.py`): доставка
>   (push/PR/report) блокируется, пока на каждую задачу нет `ground/evidence/<id>.json` с completeness ≥
>   порога (`pipeline.json evidence.threshold`, дефолт 0.95).
> - **Cost circuit breaker** (`cost-breaker`): token budget — варн на 80%, стоп на 120% (`quality.token_budget`).
> - **TDD форсится** (`tdd-guard`): запись в `src/main` блокируется, пока для задачи не написаны
>   падающие тесты (`04-test-<id>` ещё `pending`). Тесты писать можно всегда. RED строго перед кодом.
> - **Security**: `destructive-blocker`, `pii-boundary`, `prompt-guard`.
> - **State/context/stop**: `state-recorder` (пишет шаг по `step_id`), `context-injector` (grounding-excerpt),
>   `phase-gate` (не завершить с `in_progress`), `log-agent` (append-only JSONL аудит).
>
> Это **страховка поверх** дисциплины ниже: продолжай явно прогонять гейты, собирать evidence
> (`build_evidence.py`) и закрывать шаги. **Возвращай из субагентов JSON с полем `step_id`** — иначе
> `state-recorder` не пометит шаг. R4-действия требуют approval-маркер; клади его в `ground/approvals/`.
...
```bash
python ~/.gigacode/skills/pipeline-state/scripts/read.py --skill feature-pipeline --list
```
...
```bash
python ~/.gigacode/skills/pipeline-state/scripts/read.py --skill feature-pipeline --feature <slug>
```
...
| `04-build-<taskId>` | TDD GREEN: код зеленит тесты задачи | `04-test-<taskId>` |
...
`04-test-*`, `04-build-*` и `07-deliver-*` добавляются после фазы 2 через `add_steps.py` (см. §5),
...
--step-id <id> --status completed` с его JSON. Перед синтезаторами/дизайнером — выжимки через
...
>   prompt="<контракт фазы из references/subagent-prompts.md + конкретные пути/задача>"
> )
> ```
...
tests→`check_coverage.py`, document→`enrich_grounding.py` (инкрементально), deliver→`check_delivery.py`. Шаг не
...
в BRD-скилл, чтобы он встал ПЕРВОЙ строкой шапки `brd.md` (`**Jira:**`). Этот ключ протягивается
...
- **exit 0 (есть)** — переиспользуй найденный обзор, `system-analyst` НЕ запускай. Если `kind=scan`
  или `overview` без `grounding-excerpt.json` — собери выжимку (project-grounder §4). Сразу закрой
  `01-grounding` (completed, в output — `path`/`excerpt_path` из вердикта). **Не спрашивай и не
  пересканируй.**
- **exit 1 (нет)** — только тогда прочитай инструкции и запусти полный обзор:
  ```
  read_file("~/.gigacode/skills/system-analyst/SKILL.md")
  ```
  У него свой цикл и свой гейт коммита спеки.

Свежесть между фичами поддерживается инкрементально в фазе Document (`enrich_grounding.py`), поэтому
полный рескан на каждом прогоне не нужен. Обнови `01-grounding`.
...
После «да»: добавь в манифест шаги `04-test-<taskId>` (RED, при `quality.tdd:true`),
`04-build-<taskId>` (depends_on `04-test-<taskId>`) и `07-deliver-<taskId>` по `task-plan.tasks`
скриптом `add_steps.py --skill feature-pipeline --feature <slug> --steps '<...>'`
...
1. **Миграции** (если есть слой `migration`): Liquibase changeset по `tech-design §4` (см.
   `references/migrations.md`).

2. **RED — тесты вперёд (субагент-тестописатель, ЯВНЫЙ вызов `agent`).** Тесты из `acceptance`
   (Given-When-Then) + контракта tech-design; слой — **сервисные unit с моками**, `@DataJpaTest`/
   интеграционные ИЗБЕГАТЬ (см. `quality.test_layer`):
   ```
   agent(subagent_type="general-purpose", description="TDD red: tests for <taskId>",
         prompt="<контракт тестописателя §4.1 subagent-prompts.md + task.acceptance + контракт tech-design>")
   ```

3. **Стабы сигнатур (GREEN, шаг 1).** Прочитай разработчика и создай ТОЛЬКО сигнатуры целевых
   методов/классов из tech-design с телом-заглушкой (`throw new UnsupportedOperationException()`),
   чтобы тесты компилировались:
   ```
   read_file("~/.gigacode/skills/java-spring-dev/SKILL.md")
   ```
   Если есть `<project>/ground/conventions.md` — передай её (приоритетнее generic-шаблона).

4. **Gate RED — подтверди, что тесты компилируются и падают:**
   ```bash
   python3 ~/.gigacode/skills/feature-pipeline/scripts/check_tests_red.py \
       --root . --pipeline-config "<project>/ground/pipeline.json" --test-filter "<TaskTest>"
   ```
   `fail (не компилится)` → доведи стабы; `fail (зелёные)` → тест слабый, верни тестописателю.
   Только при `pass` (компилируется + падает) закрой `04-test-<taskId>` и иди дальше.

5. **GREEN — реализация (java-spring-dev).** Допиши тела по `task.layers`
   (entity→repo→dto→mapper→service→controller — только из плана), **минимально**, пока RED-тесты
   не позеленеют. Не добавляй слои «на всякий случай».

6. **Gate перед закрытием `04-build-<taskId>`:**
   ```bash
   python3 ~/.gigacode/skills/feature-pipeline/scripts/check_build.py \
       "<папка фичи>/task-plan.json" --task <taskId>
   ```
   Артефакты на диске + тесты задачи зелёные. Обнови `04-build-<taskId>` (completed + файлы) при `pass`.

> Если `quality.tdd: false` — допускается старый порядок (код → тесты в фазе Verify), но это деградация.

---

## 8. Фаза 4 — Verify (полный прогон + покрытие)

При TDD тесты задач уже написаны и зелёные (фаза 3). Verify = **полный прогон всей фичи + покрытие**;
тестописателя зовём только для **добора покрытия** там, где `check_coverage` дал `LOW`/`MISSING`
(не переписывая уже зелёные тесты). Оба — ЯВНЫЙ вызов `agent` (промпты — [`references/subagent-prompts.md`](references/subagent-prompts.md)):
1. **тестописатель (добор)** — дописывает тесты только под непокрытое:
   ```
   agent(subagent_type="general-purpose", description="cover gaps for <фича>",
         prompt="<контракт тестописателя + список LOW/MISSING от check_coverage>")
   ```
   (Если TDD выключен — здесь пишутся тесты с нуля, как раньше.)
2. **тестраннер** — гоняет `./gradlew test jacocoTestReport`, затем покрытие проверяет
   **детерминированно** (не глазами LLM):
   ```
   agent(subagent_type="general-purpose", description="run tests + coverage",
         prompt="<контракт тестраннера + команда сборки и check_coverage.py>")
   ```
   Внутри тестраннер выполняет:

```bash
python3 ~/.gigacode/skills/minor-defect-fix/scripts/check_coverage.py \
    --base "<база ветки фичи>" \
...
Спецадаптер — **субагент, запусти явным вызовом `agent`** (не правь спеку сам):
```
agent(subagent_type="general-purpose", description="update spec for <фича>",
      prompt="<контракт спецадаптера из subagent-prompts.md + список затронутых .md и diff>")
```
Он обновляет затронутые `.md` в `docs/` (включая `system-analysis/api.md`, если добавились
эндпойнты) в ветке репо спеки, **без push** до гейта доставки. Опционально — `java-uml-spec`
перерисовывает диаграммы затронутых контроллеров.

**Gate перед закрытием `06-spec` — инкрементальное обогащение ground (НЕ полный рескан):**
подмешай в ground только дельту фичи (изменённые модули), чтобы следующая фича стартовала с
актуального среза. Обзор `system-analysis/` обновляется по изменённым модулям, нетронутые остаются
как есть; `grounding-excerpt.json` пересобирается, полнота проверяется `verify_coverage.py`:
```bash
python3 ~/.gigacode/skills/system-analyst/scripts/enrich_grounding.py \
    --scan "docs/system-analysis/scan" \
    --task-plan "<папка фичи>/task-plan.json"
```
(Дельта берётся из `task-plan.tasks[].artifacts`; можно `--changed-files …` или `--git-diff <base>`.)
Изменённые `scan/*.json` и `grounding-excerpt.json` коммитятся вместе с фичей в фазе Deliver.
Если `enrich_grounding.py` вернул exit 2 (verify_coverage не сошёлся) — есть рассинхрон, сделай
**полный** рескан вручную (`scan_all.py -o docs/system-analysis/scan`). Затем обнови `06-spec`.
