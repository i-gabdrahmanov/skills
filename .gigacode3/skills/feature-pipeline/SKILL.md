...
> **Control plane на хуках — harness PDLC v3.5 (gigacode3).** Enforcement форсится рантайм-хуками
...
> При инициализации положи в `--context` поля `feature` (slug фичи или Jira-ключ, напр.
> `npf-login`) и `iteration` (номер прогона) — по ним хук `agent-logger` группирует живые
> логи субагентов в `ground/ai-logs/<feature>/iter-NN/`. Необязательно, но даёт читаемые имена.
...
скриптом `~/.gigacode/skills/pipeline-state/scripts/add_steps.py` (идемпотентно, манифест руками не правь).
