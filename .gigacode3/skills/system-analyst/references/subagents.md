...
Готовый scan-JSON по твоей категории (полный список из scripts/scan_all.py):
  <вставь содержимое scan/<category>.json — это твой floor, его надо ОБОГАТИТЬ, а не урезать>
```

## Правило полноты (НЕ усекать!)

Источник истины по механическим категориям — детерминированный scan-JSON
(`scripts/scan_all.py`, см. SKILL.md §2.5). Субагенты этапа 2 работают по принципу
**обогащения, а не перечисления с нуля**:

> "Вот ПОЛНЫЙ список элементов из scan-JSON по твоей категории. Пройди по нему и
>  добавь смысл (описание, назначение, связи). Грепы ниже — это кросс-чек: если в
>  коде найдёшь то, чего нет в списке — ДОБАВЬ. НИЧЕГО не выкидывай и не сокращай.
>  Верни ПОЛНЫЙ список (все элементы, со всеми полями)."

Объём больше не проблема: механику уже извлёк Python без лимитов токенов, субагент
получает готовый список и не читает 50+ файлов сам.

> ⚠️ **Никакого compact-усечения, «топ-10» и «остальное кратко».** Именно это
> приводило к молчаливому недосчёту — находилось ~20% (14 из 55 entities, 21 из 93
> endpoints). Полнота проверяется гейтом `verify_coverage.py`: если в MD меньше
> элементов, чем в scan-JSON, шаг падает с `fail` и списком пропущенного.
