...
`~/.gigacode/skills/minor-defect-fix/config.json`. Путь спеки — это **корень git-репо
...
```
read_file("~/.gigacode/skills/defect-analyzer/SKILL.md")
```
...
```
read_file("~/.gigacode/skills/bugfix-developer/SKILL.md")
```
...
2. Покрытие НЕ считай вручную — прогони детерминированный gate (он сам берёт изменённые
   ...
   `python3 ~/.gigacode/skills/minor-defect-fix/scripts/check_coverage.py --root <toplevel> --base HEAD --threshold 0.80 --json`
