...
echo "== smoke-cli: harness home=$HOME_DIR, live=$LIVE =="
...
if [ -x "$HOME_DIR/hooks/doctor.py" ] || [ -f "$HOME_DIR/hooks/doctor.py" ]; then
  python3 "$HOME_DIR/hooks/doctor.py" --home "$HOME_DIR" >/tmp/_smoke_doc 2>&1 && ok "doctor: harness готов" || { no "doctor нашёл проблемы"; tail -3 /tmp/_smoke_doc | sed 's/^/      /'; }
...
MODEL_OK=false
if $LIVE; then
  if _to 120 "$CLI" -p "Ответь одним словом: ok" >/tmp/_smoke_m 2>&1; then
...
if $LIVE && $MODEL_OK; then
  ...
  ( cd "$P1" && _to 150 "$CLI" -p "Выполни в shell ровно одну команду: echo smoke-ok" --approval-mode yolo >/dev/null 2>&1 )
  ...
  ( cd "$P2" && _to 200 "$CLI" -p "Через инструмент agent (subagent_type=general-purpose) запусти субагента, который выполнит echo from-subagent." --approval-mode yolo >/dev/null 2>&1 )
  ...
  ( cd "$P3" && git init -q . && _to 150 "$CLI" -p "Сделай git commit -m smoke (пустой коммит --allow-empty)" --approval-mode yolo >/dev/null 2>&1 )
  ...
  echo "    cd /tmp/x && $CLI -p \"выполни shell: echo smoke-ok\" --approval-mode yolo   # → ground/ai-logs/**/agents.jsonl"
  echo "    $CLI -p \"через agent (general-purpose) сделай echo\" --approval-mode yolo    # → SubagentStart в логе"
fi
