#!/usr/bin/env python3
"""Мигрирует дерево скиллов Claude -> GigaCode по правилам из mapping.json.
...
Примеры:
  ...
  # реальная миграция ~/.claude/skills -> ~/.gigacode/skills
  python migrate.py
  # своя пара путей
  python migrate.py --source ~/.claude --target ~/.gigacode
"""
import argparse
import json
import os
...
def main():
    ap = argparse.ArgumentParser(description="Claude -> GigaCode migrator")
    home = os.path.expanduser("~")
    ap.add_argument("--source", default=os.path.join(home, ".claude", "skills"),
                    help="что мигрировать (default: ~/.claude/skills)")
