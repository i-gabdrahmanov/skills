...
3. **Evidence-инвалидация**: запись `kind:"reopen"` в журнал прогона гасит origin/гейт
   шага и перечисленные в ней вердикты судей и overrides (файлы старой раскладки —
...
Дополнительно: динамические шаги (`04-test-*`/`04-build-*`) при откате фазы
...
- **Jira не трогается**: rollback печатает список сирот (Story/сабтаски из
  `jira-tasks-result.json`) и черновик комментария в Story — постить и убирать решает
  человек. Коммиты/ветки/PR пайплайн не создаёт, так что git-сирот у отката нет.
