...
| `scripts/add_steps.py` | Дописать новые шаги в существующий manifest (идемпотентно; для шагов, известных только по ходу прогона). Проставляет `required_judges` по единой маске. Фазовую машину синхронизировать не нужно — она выводится из манифеста. **Для feature-pipeline канон — `skills/feature-pipeline/scripts/add_steps.py`**; эта generic-версия — для прочих скиллов. |
