...
> **Исторический срез (2026-07-04).** Документы описывают раскладку того времени — проектный
> деплой `deploy.sh` + `deploy-local.sh` + `resolve_hook_paths.py` и merge блока hooks в
> `settings.json` проекта. Сейчас Forge поставляется нативным extension'ом: проводка живёт в
> `hooks/hooks.json` через `${CLAUDE_PLUGIN_ROOT}`, установщика и резолвера путей нет.
> Анализ поверхности конфига и векторов обхода остаётся в силе — читайте с поправкой на способ
> доставки. Актуальная установка — [INSTALL.md](../../INSTALL.md).
